...
@dataclass
class PPOConfig:
    """PPO hyperparameters (exact match with Table 2)."""
    learning_rate: float = 3e-4           # Adam optimizer step size
    rollout_length: int = 2048            # Environment steps per update  
    batch_size: int = 64                  # Size of mini-batches for gradient updates
    epochs_per_update: int = 10           # Number of passes over each batch
    clip_range: float = 0.2               # PPO clipping parameter
    discount_factor: float = 0.99         # Weighting of future rewards
    gae_lambda: float = 0.95              # GAE parameter
    entropy_coefficient: float = 0.01     # Encourages exploration
