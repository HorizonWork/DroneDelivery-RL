...
class CurriculumTrainer:
    ...
    def _phase_completion_check(self, phase: CurriculumPhase, phase_result: Dict[str, Any]) - bool:
        ...
        success_rate = phase_result.get('final_success_rate', 0.0) / 100
        ...
        if criteria_met:
            self.logger.info(f"{phase.value} completion criteria met: {success_rate100:.1f}  {threshold100:.1f}")
        else:
            self.logger.warning(f"{phase.value} completion criteria NOT met: {success_rate100:.1f}  {threshold100:.1f}")
