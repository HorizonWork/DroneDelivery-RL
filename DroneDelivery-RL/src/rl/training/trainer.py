...
class PPOTrainer:
    ...
    def __init__(self, agent: PPOAgent, environment, config: Dict[str, Any]):
        ...
        self.logger.info(f"Target success rate: {self.training_config.target_success_rate}")
    ...
    def train(self) -> Dict[str, Any]:
        ...
        if self.training_config.resume_from_checkpoint and self.training_config.checkpoint_path:
        ...
        while self.state.timestep < self.training_config.total_timesteps:
            ...
            # Periodic evaluation
            if (self.state.timestep % self.training_config.eval_frequency == 0 or
    ...
    def _run_training_episode(self) -> Dict[str, Any]:
        ...
        while not done:
            ...
            # Record experience
            self.agent.add_experience(observation, action, reward, value, log_prob, done)
    ...
    def _should_stop_early(self) -> bool:
        ...
        # Target achieved
        if (self.state.best_success_rate >= self.training_config.target_success_rate * 100):
            self.logger.info(f"Target success rate {self.training_config.target_success_rate*100}% achieved")
            return True
        
        # No improvement for too long
        if self.state.steps_since_improvement >= self.training_config.early_stopping_patience:
            self.logger.info(f"No improvement for {self.training_config.early_stopping_patience:,} steps")
            return True
    ...
    def _record_episode_metrics(self, episode_result: Dict[str, Any]):
        ...
        self.training_metrics['episode_rewards'].append(episode_result['episode_reward'])
        self.training_metrics['episode_lengths'].append(episode_result['episode_length'])
        
        if episode_result['success']:
            self.training_metrics['energy_consumptions'].append(episode_result['episode_energy'])
    ...
    def _log_training_progress(self):
        ...
        recent_success = list(self.training_metrics['success_rates'])[-10:] if self.training_metrics['success_rates'] else [0]
        ...
        self.logger.info(f"[{progress:5.1f}%] Episode {self.state.episode:,}, "
                        f"Timestep {self.state.timestep:,}/{self.training_config.total_timesteps:,}")
        self.logger.info(f"  Reward: {avg_reward:8.1f}, Length: {avg_length:6.1f}, Success: {avg_success:5.1f}%")
        
        # Log losses if available
        if self.training_metrics['policy_losses']:
            recent_policy_loss = np.mean(list(self.training_metrics['policy_losses'])[-10:])
            recent_value_loss = np.mean(list(self.training_metrics['value_losses'])[-10:])
            self.logger.info(f"  Policy Loss: {recent_policy_loss:.4f}, Value Loss: {recent_value_loss:.4f}")
        ...
        if self.use_wandb:
            ...
            if self.training_metrics['energy_consumptions']:
                avg_energy = np.mean(list(self.training_metrics['energy_consumptions'])[-50:])
    ...
    def _save_checkpoint(self):
        ...
        checkpoint_data = {
            ...
            'training_metrics': {
                ...
                'energy_consumptions': list(self.training_metrics['energy_consumptions'])
            },
            'training_config': self.training_config.__dict__
        }
    ...
    def _load_checkpoint(self, checkpoint_path: str):
        ...
        try:
            ...
            self.state.steps_since_improvement = training_state['steps_since_improvement']
            ...
            if 'training_metrics' in checkpoint_data:
                ...
                self.training_metrics['episode_rewards'].extend(metrics.get('episode_rewards', []))
                self.training_metrics['episode_lengths'].extend(metrics.get('episode_lengths', []))
                self.training_metrics['success_rates'].extend(metrics.get('success_rates', []))
                self.training_metrics['energy_consumptions'].extend(metrics.get('energy_consumptions', []))
            
            self.logger.info(f"Training resumed from {checkpoint_path}")
            self.logger.info(f"Resuming from timestep {self.state.timestep:,}, episode {self.state.episode:,}")
    ...
    def _final_evaluation(self) -> Dict[str, Any]:
        ...
        final_summary = full_evaluator.evaluate_policy(self.agent, self.environment, "PPO_Final")
    ...
    def _save_final_model(self):
        ...
        summary_path = self.checkpoint_dir / f"{self.training_config.experiment_name}_summary.json"
        
        training_summary = {
            'experiment_name': self.training_config.experiment_name,
            'total_timesteps': self.state.timestep,
            'total_episodes': self.state.episode,
            'training_time_hours': (time.time() - self.state.training_start_time) / 3600,
            'best_success_rate': self.state.best_success_rate,
            'best_energy_efficiency': self.state.best_energy_efficiency,
    ...
    def get_training_statistics(self) -> Dict[str, Any]:
        ...
        training_time = time.time() - self.state.training_start_time if self.state.training_start_time > 0 else 0
        
        return {
            'training_progress': {
                'current_timestep': self.state.timestep,
                'current_episode': self.state.episode,
                'progress_percent': (self.state.timestep / self.training_config.total_timesteps) * 100,
            ...
            'performance': {
                'best_success_rate': self.state.best_success_rate,
                'best_energy_efficiency': self.state.best_energy_efficiency,
                'recent_avg_reward': np.mean(list(self.training_metrics['episode_rewards'])[-20:]) if self.training_metrics['episode_rewards'] else 0,
                'recent_success_rate': np.mean(list(self.training_metrics['success_rates'])[-5:]) * 100 if self.training_metrics['success_rates'] else 0
            },
            'training_stability': {
                'steps_since_improvement': self.state.steps_since_improvement,
                'current_learning_rate': self.state.current_lr,
                'policy_loss_trend': np.mean(list(self.training_metrics['policy_losses'])[-10:]) if self.training_metrics['policy_losses'] else 0,
                'value_loss_trend': np.mean(list(self.training_metrics['value_losses'])[-10:]) if self.training_metrics['value_losses'] else 0
            }
        }
