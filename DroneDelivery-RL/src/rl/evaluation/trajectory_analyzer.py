...
class TrajectoryAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.smoothness_window = config.get('smoothness_window', 5)
        self.velocity_smoothing = config.get('velocity_smoothing', 0.1)
        ...
        self.maneuver_acceleration_threshold = config.get('maneuver_threshold', 1.0)
    ...
    def _calculate_smoothness(self, trajectory: np.ndarray, timestamps: np.ndarray) - float:
        ...
        if len(accelerations)  1:
            ...
            smoothness = 1.0 / (1.0 + mean_jerk / 3.0)
    ...
    def _analyze_floor_transitions(self, trajectory: np.ndarray) - float:
        ...
        min_vertical_distance = abs(altitudes[-1] - altitudes[0])
    ...
    def _assess_trajectory_quality(self, trajectory_metrics: List[TrajectoryMetrics]) - Dict[str, Any]:
        ...
        for metrics in trajectory_metrics:
            path_eff_score = min(1.0, metrics.path_efficiency / 0.8)
        ...
        return {
            ...
            'improvement_potential': max(0.0, 0.85 - mean_quality)
        }
