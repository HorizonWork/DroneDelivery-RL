...
class ExperienceBuffer:
    ...
    def add(self, observation: np.ndarray, action: np.ndarray, reward: float,
           next_observation: np.ndarray, done: bool, log_prob: float = 0.0,
           value: float = 0.0):
    ...
    def compute_gae(self, gae_lambda: float, discount_factor: float,
                   next_value: float, next_done: bool):
        ...
        for t in reversed(range(self.size)):
            ...
            # TD error
            td_error = self.rewards[t] + discount_factor * next_val * next_non_terminal - self.values[t]
            
            # GAE
            advantages[t] = td_error + discount_factor * gae_lambda * next_advantage * next_non_terminal
...
class PrioritizedReplayBuffer:
    ...
    def __init__(self, capacity: int, observation_dim: int, action_dim: int,
                 alpha: float = 0.6, beta_start: float = 0.4, beta_steps: int = 1_000_000):
        self.capacity = capacity
        ...
        self.logger = logging.getLogger(__name__)
        self.logger.info(f"Prioritized Replay Buffer initialized: {capacity:,} capacity")
    ...
    def sample_batch(self, batch_size: int) -> Tuple[Dict[str, torch.Tensor], np.ndarray, np.ndarray]:
        ...
        current_beta = min(1.0, self.beta_start + (1.0 - self.beta_start) * self.total_steps / self.beta_steps)
