...
class TargetManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.target_height_offset = config.get('target_height_offset', 0.2)  # 0.2m above floor
        ...
        self.logger.info(f"Target Manager initialized: {len(self.targets)} targets across {self.num_floors} floors")
        self.logger.info(f"Targets per floor: {self.targets_per_floor}, Curriculum enabled: {self.curriculum_enabled}")
    ...
    def _generate_random_position(self, floor: int) -> Tuple[float, float, float]:
        ...
        x = random.uniform(
            self.wall_clearance,
            self.floor_dimensions['length'] - self.wall_clearance
        )
        y = random.uniform(
            self.wall_clearance,
            self.floor_dimensions['width'] - self.wall_clearance
        )
        
        return (x, y, target_z)
    
    def _determine_target_difficulty(self, floor: int, 
                                   position: Tuple[float, float, float]) -> TargetDifficulty:
        """
        ...
        is_corner = (
            (x < corner_threshold or x > self.floor_dimensions['length'] - corner_threshold) and
            (y < corner_threshold or y > self.floor_dimensions['width'] - corner_threshold)
        )
    ...
    def select_target(self, curriculum_phase: Optional[int] = None,
                     difficulty_filter: Optional[TargetDifficulty] = None) -> LandingTarget:
        """
        ...
        if difficulty_filter is not None:
            available_targets = [t for t in available_targets 
                               if t.difficulty == difficulty_filter]
        
        # Select random target from available options
        if not available_targets:
            self.logger.warning("No available targets matching criteria, using fallback")
        ...
        self.logger.info(f"Selected target: {selected_target.name} on floor {selected_target.floor} "
    ...
    def get_targets_by_difficulty(self, difficulty: TargetDifficulty) -> List[LandingTarget]:
        """
    ...
    def is_at_target(self, position: Tuple[float, float, float], 
