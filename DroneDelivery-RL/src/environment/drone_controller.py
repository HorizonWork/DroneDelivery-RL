...
class DroneController:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.min_altitude = config.get('min_altitude', 0.5)
        self.max_altitude = config.get('max_altitude', 20.0)
        ...
        self.command_timeout = config.get('command_timeout', 0.5)

        self.controller_state = ControllerState()
        self.last_drone_status: Optional[DroneStatus] = None
        self.smoothed_velocity = np.zeros(4)

        self.max_acceleration = config.get('max_acceleration', 10.0)
        self.max_jerk = config.get('max_jerk', 20.0)
