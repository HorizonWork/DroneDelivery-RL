...
class AStarBaseline:
    ...
    def plan(self, start: Tuple[float, float, float],
             ...
             obstacles: Optional[List] = None) - List[Tuple[float, float, float]]:

        if obstacles:
            self.planner.update_occupancy_grid(obstacles)
        return self.planner.plan_path(start, goal)

    def control(self, current_state: Dict[str, Any],
                target_waypoint: Tuple[float, float, float]) - Dict[str, Any]:
