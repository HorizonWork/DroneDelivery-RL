...
class PathSmoother:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.method = config.get('smoothing_method', 'spline')
        self.spline_degree = config.get('spline_degree', 3)
        self.smoothness_factor = config.get('smoothness_factor', 0.1)

        self.target_point_spacing = config.get('target_spacing', 0.5)
    ...
    def _smooth_with_bezier(self, path: List[Tuple[float, float, float]]) - List[Tuple[float, float, float]]:
        ...
        for i in range(0, len(path) - segment_size + 1, segment_size - 1):
            ...
            bezier_points = self._generate_bezier_curve(segment, 10)
