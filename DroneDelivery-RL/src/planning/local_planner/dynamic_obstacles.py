...
class DynamicObstacleTracker:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.velocity_smoothing = config.get('velocity_smoothing', 0.7)
        self.position_noise_std = config.get('position_noise_std', 0.1)

        self.default_obstacle_radius = config.get('default_obstacle_radius', 0.3)
        self.max_velocity = config.get('max_velocity', 2.0)
        self.max_acceleration = config.get('max_acceleration', 1.0)

        self.max_association_distance = config.get('max_association_distance', 1.0)
