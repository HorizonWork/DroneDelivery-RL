...
class CoordinateTransformer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.body_to_camera_offset = np.array(config.get('body_to_camera_offset', [0.1, 0.0, -0.05]))
        self.slam_to_world_transform = np.eye(4)

        self.grid_resolution = config.get('grid_resolution', 0.5)
        self.grid_origin = np.array(config.get('grid_origin', [0, 0, 0]))
...
class FrameConverter:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.use_enu = config.get('use_enu_convention', True)
