...
class BaselineBenchmark:
    ...
    def run_benchmark(self) -> Dict[str, Any]:
        ...
        for baseline_name, baseline_config in self.baselines.items():
            ...
            self.logger.info(f"{baseline_name}: {success_rate:.1f}% success, {mean_energy:.0f}J energy")
        ...
        self.logger.info(f"Baseline benchmark completed in {benchmark_time/60:.1f} minutes")
        return final_results
    
    def _evaluate_baseline(self, baseline_name: str, baseline_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for episode in range(self.num_episodes):
            if episode % 10 == 0:
                self.logger.info(f"{baseline_name} episode {episode}/{self.num_episodes}")
    ...
    def _run_baseline_episode(self, baseline_name: str, baseline_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        while not done and steps < max_steps:
            ...
            if baseline_config.get('use_random_policy', False):
                ...
                action = self._astar_only_action(planned_path, steps, start_pos, goal_pos)
            else:  # RRT+PID
    ...
    def _astar_only_action(self, planned_path: List, step: int, start_pos: np.ndarray, 
    ...
    def _rrt_pid_action(self, global_path: List, observation: np.ndarray, 
                       goal_pos: np.ndarray) -> np.ndarray:
        """
        ...
        try:
            ...
            if global_path and len(global_path) > 1:
                ...
                distances = [np.linalg.norm(np.array(wp) - current_pos) for wp in global_path]
                next_waypoint_idx = min(range(len(distances)), key=distances.__getitem__)
                local_goal = global_path[min(next_waypoint_idx + 1, len(global_path) - 1)]
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        summary['method_rankings']['by_success_rate'] = [method for method, _ in sorted_by_success]
...
def main():
    ...
    parser.add_argument('--output', type=str, default='results/baseline_benchmark.json',
