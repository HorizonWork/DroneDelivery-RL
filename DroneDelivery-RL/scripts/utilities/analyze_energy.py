...
class EnergyAnalyzer:
    ...
    def analyze_training_energy(self, training_results_file: str) -> Dict[str, Any]:
        ...
        # Extract energy data
        energy_history = training_data.get('training_history', {}).get('episode_energies', [])
        ...
        optimization_opportunities = self._identify_optimization_opportunities(energy_history)
    ...
    def analyze_evaluation_energy(self, evaluation_results_file: str) -> Dict[str, Any]:
        ...
        efficiency_analysis = {
            ...
            'energy_consistency': self._calculate_energy_consistency(std_energy, mean_energy),
        ...
        evaluation_energy_analysis = {
            ...
            'recommendations': self._generate_energy_recommendations(mean_energy, std_energy)
        }
        
        self.logger.info(f"Evaluation energy analysis: {mean_energy:.0f}J ± {std_energy:.0f}J")
        return evaluation_energy_analysis
    
    def _analyze_phase_energy_progression(self, training_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, (phase_name, end_idx) in enumerate(zip(phases, phase_boundaries)):
            ...
            if phase_energies:
                phase_analysis[phase_name] = {
                    ...
                    'energy_trend': 'decreasing' if i > 0 and np.mean(phase_energies) < np.mean(energy_history[max(0, start_idx-100):start_idx]) else 'stable',
                    'efficiency_improvement': self._calculate_phase_efficiency_improvement(phase_energies, i)
                }
        ...
        return {
            ...
            'overall_progression': self._analyze_overall_energy_progression(phase_analysis)
        }
    
    def _calculate_efficiency_trends(self, energy_history: List[float]) -> Dict[str, Any]:
        ...
        return {
            ...
            'trend_direction': 'improving' if energy_improvement > 0 else 'stable' if abs(energy_improvement) < 5 else 'degrading'
        }
    
    def _analyze_energy_distribution(self, energy_history: List[float]) -> Dict[str, Any]:
    ...
    def _compare_with_baselines(self, energy_history: List[float]) -> Dict[str, Any]:
        ...
        # Current performance
        recent_energy = np.mean(energy_history[-100:]) if len(energy_history) >= 100 else np.mean(energy_history)
        
        # Calculate savings vs each baseline
        savings_analysis = {}
        for baseline_name, baseline_energy in self.analysis_config['baseline_energy'].items():
            ...
            savings_analysis[baseline_name] = {
                ...
                'target_met': savings_percent >= 25.0 if baseline_name == 'A_star_only' else True
            }
        
        # Overall efficiency rating
        max_savings = max(s['energy_savings_percent'] for s in savings_analysis.values())
        efficiency_rating = 'excellent' if max_savings >= 50 else 'good' if max_savings >= 25 else 'fair' if max_savings >= 10 else 'poor'
        
        return {
            'baseline_comparisons': savings_analysis,
            'maximum_savings_percent': max_savings,
            'efficiency_rating': efficiency_rating,
            'primary_target_met': savings_analysis.get('A_star_only', {}).get('target_met', False)
        }
    
    def _identify_optimization_opportunities(self, energy_history: List[float]) -> List[Dict[str, Any]]:
        ...
        recent_energy = np.mean(energy_history[-100:]) if len(energy_history) >= 100 else np.mean(energy_history)
        energy_std = np.std(energy_history[-100:]) if len(energy_history) >= 100 else np.std(energy_history)
    ...
    def _calculate_battery_life_impact(self, mean_energy: float) -> Dict[str, Any]:
        ...
        missions_per_day = missions_per_charge / 3  # Conservative estimate (charging time)
        ...
        return {
            ...
            'battery_efficiency': 'excellent' if missions_per_charge >= 50 else 'good' if missions_per_charge >= 30 else 'fair' if missions_per_charge >= 20 else 'poor'
        }
    ...
    def _generate_energy_summary(self, energy_history: List[float]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'energy_consistency': self._calculate_energy_consistency_score(np.array(energy_history)),
            'overall_assessment': self._generate_overall_energy_assessment(recent_energy, efficiency_grade)
        }
    ...
    def _assess_energy_targets(self, energy: float) -> Dict[str, bool]:
        ...
        return {
            ...
            'better_than_rrt_pid': energy < self.analysis_config['baseline_energy']['RRT_PID']
        }
    ...
    def _calculate_energy_consistency_score(self, energy_array: np.ndarray) -> float:
        ...
        cv = np.std(energy_array) / np.mean(energy_array) if np.mean(energy_array) > 0 else 1.0
    ...
    def generate_energy_visualizations(self, analysis_results: Dict[str, Any], 
                                     output_dir: str):
        ...
        try:
            ...
            if 'efficiency_trends' in analysis_results:
                self._plot_energy_trends(analysis_results['efficiency_trends'], output_path)
            
            # Energy distribution plot
            if 'distribution_analysis' in analysis_results:
                self._plot_energy_distribution(analysis_results['distribution_analysis'], output_path)
            
            # Baseline comparison plot
            if 'baseline_comparison' in analysis_results:
                self._plot_baseline_comparison(analysis_results['baseline_comparison'], output_path)
...
def main():
    ...
    parser.add_argument('--output', type=str, default='results/energy_analysis',
    ...
    if args.evaluation_results:
        evaluation_analysis = analyzer.analyze_evaluation_energy(args.evaluation_results)
    ...
    if args.visualize:
        for analysis_type, analysis_data in results.items():
            analyzer.generate_energy_visualizations(analysis_data, str(output_dir / analysis_type))
    ...
    if 'evaluation_analysis' in results:
        ...
        print(f"Energy Savings vs A*: {eval_efficiency['baseline_comparison']['A_star_only']['energy_savings_percent']:.1f}%")
        print(f"Target Met: {'✅ YES' if eval_efficiency['baseline_comparison']['A_star_only']['target_met'] else '❌ NO'}")
