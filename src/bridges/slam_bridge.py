...
class SLAMBridge:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.slam_config_path = config.get(
            "slam_config_path", "config/slam/orb_slam3_config.yaml"
        )
        self.vocabulary_path = config.get("vocabulary_path", "data/slam/ORBvoc.txt")
        self.camera_config_path = config.get(
            "camera_config_path", "config/slam/camera_calibration.yaml"
        )
        self.imu_config_path = config.get(
            "imu_config_path", "config/slam/imu_calibration.yaml"
        )

        # Output paths
        self.trajectory_output = config.get(
            "trajectory_output", "data/slam/trajectories/trajectory.tum"
        )
    ...
    def _process_visual_frame(
        self, left_img: np.ndarray, right_img: np.ndarray, timestamp: float
        ...
        try:
            ...
            K = np.array(
                [
                    [
                        self.camera_params["left"]["fx"],
                        0,
                        self.camera_params["left"]["cx"],
                    ],
                    [
                        0,
                        self.camera_params["left"]["fy"],
                        self.camera_params["left"]["cy"],
                    ],
                    [0, 0, 1],
                ]
            )
    ...
    def _update_slam_state(self):
        ...
        if (
            current_time - self.last_processed_image_time > 1.0
            and self.slam_state.is_tracking
        ):
