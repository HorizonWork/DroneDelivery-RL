...
class AirSimBridge:
    ...
    def _ensure_str_keys(obj: Any) -> Any:
        if isinstance(obj, dict):
            return { (k.decode('utf-8') if isinstance(k, bytes) else k): AirSimBridge._ensure_str_keys(v) for k, v in obj.items() }
    ...
    def takeoff(self, altitude: float = 1.0, timeout: float = 15.0) -> bool:
        ...
        try:
            ...
            self.client.moveToZAsync(-altitude, 1.0, timeout_sec=timeout, 
