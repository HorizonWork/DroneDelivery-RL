...
# Initialize occupancy grid
    ...
    def capture_sensor_snapshot(self) -> Optional[SensorReading]:
        ...
        if frames["depth"] is None and frames["left"] is None and frames["right"] is None:
    ...
    def get_camera_data(self) -> Optional[Dict[str, np.ndarray]]:
        ...
        with self.data_lock:
            if self.latest_reading:
                return {
                    "left": self.latest_reading.stereo_left.copy()
                    if self.latest_reading.stereo_left is not None
                    else None,
                    "right": self.latest_reading.stereo_right.copy()
                    if self.latest_reading.stereo_right is not None
                    else None,
                    "depth": self.latest_reading.depth_image.copy()
