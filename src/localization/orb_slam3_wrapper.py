...
class ORBSLAM3Wrapper:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.lib_path = config.get('orb_slam3_lib_path', '/usr/local/lib/libORB_SLAM3.so')
        self.vocabulary_path = config.get('vocabulary_path', 'data/slam/ORBvoc.txt')
        self.settings_file = config.get('settings_file', 'config/slam/stereo_inertial.yaml')
        
        # Initialize coordinate transforms
        self.coord_transforms = CoordinateTransforms(config.get('transforms', {}))
        
        # System configuration
        self.sensor_type = config.get('sensor_type', 'STEREO_INERTIAL')  # STEREO, MONOCULAR, STEREO_INERTIAL
    ...
    def _create_settings_file(self):
        ...
        settings = {
            ...
            # Stereo parameters
            'Camera.bf': self.config.get('fx', 460.0) * self.config.get('baseline', 0.10),
            'Stereo.ThDepth': 50.0,
    ...
    def process_imu_measurement(self, accel: np.ndarray, gyro: np.ndarray, timestamp: float):
        """
