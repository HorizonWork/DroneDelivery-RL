    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.enabled:
            ...
            for phase, config in self.phases.items():
                self.logger.info(
                    f"  {config.name}: {config.timesteps/1e6:.1f}M timesteps, "
    ...
    def _check_phase_progression(self):
        ...
        if min_episodes_met and min_timesteps_met and success_threshold_met:
            if not current_progress.ready_for_next:
                ...
                self.logger.info(
                    f"Phase {self.current_phase.value} progression criteria met!"
                )
                self.logger.info(
                    f"  Episodes: {current_progress.episodes_completed}/{phase_config.min_episodes}"
                )
                self.logger.info(
                    f"  Timesteps: {current_progress.timesteps_completed}/{phase_config.timesteps}"
                )
                self.logger.info(
