...
Supports both AirSim LiDAR scanning and synthetic obstacle generation.

FIXED ISSUES:
- Correct AirSim API usage (no simTestLineOfSightBetweenPoints)
- Proper NED ↔ ENU coordinate conversion
- Efficient LiDAR point cloud processing
- Better error handling and logging
- Memory-efficient grid inflation
"""
...
class FlyZoneBuilder:
    """Main class for building 3D occupancy grids"""
    ...
    def _ned_to_enu(self, x_ned: float, y_ned: float, z_ned: float) -> Tuple[float, float, float]:
        """Convert NED (AirSim world frame) to ENU (our grid frame)
        
        Coordinate systems:
        - AirSim/Unreal (NED): X=North, Y=East, Z=Down
        - Our Grid (ENU): X=East, Y=North, Z=Up
        - Unreal world offset: {60, -30, 1.5} m from PlayerStart rebase
        
        Steps:
        1. AirSim NED (with world offset) → Remove offset → NED origin
        2. NED origin → Rotate to ENU
        """
        # Remove world offset (AirSim coordinates already include this)
    ...
    def _enu_to_ned(self, x_enu: float, y_enu: float, z_enu: float) -> Tuple[float, float, float]:
        """Convert ENU (our grid frame) to NED (AirSim world frame)
        
        Steps:
        1. ENU → Rotate to NED origin
        2. NED origin → Add world offset → AirSim NED
        """
    ...
    def _inflate_obstacles(self, grid: np.ndarray) -> np.ndarray:
        """Inflate obstacles by drone radius for safety margin"""
    ...
    def _get_airsim_lidar_grid(self) -> Optional[np.ndarray]:
        """Build occupancy grid from AirSim using LiDAR scans
        
        FIXED ISSUES:
        - Proper coordinate conversion (NED ↔ ENU)
        - Efficient point cloud processing
        - Batch grid updates instead of per-point
        """
        ...
        try:
            ...
            # Generate scan positions in ENU (our grid frame)
            ...
            total_points = 0
            occupied_points = []  # Collect all occupied points for batch processing
            
            for idx, (x_enu, y_enu, z_enu) in enumerate(scan_positions_enu):
                try:
                    # Convert scan position to NED for AirSim
                    ...
                    time.sleep(0.05)  # Let physics settle
                    ...
                    # Parse point cloud (format: [x1,y1,z1, x2,y2,z2, ...] in NED relative to drone)
    ...
    def _has_obstacle_collision_check(self, x: float, y: float, z: float) -> bool:
        """Check if position has obstacle using collision detection (DEPRECATED)
        
        NOTE: This method is NOT used in the new LiDAR-based approach.
        ...
        try:
            ...
            # Move drone to position and check collision
    ...
    def build_grid(self) -> np.ndarray:
        """Build occupancy grid using AirSim LiDAR or synthetic obstacles"""
        self.logger.info("=" * 60)
        ...
        # Try AirSim LiDAR first if enabled
        ...
        # Fallback to synthetic obstacles
        self.logger.info("=" * 60)
        ...
        self.logger.info(f"  Free cells: {free_count}")
        self.logger.info(f"  Occupied cells: {occupied_count}")
        self.logger.info(f"  Free space: {free_count/total_cells*100:.1f}%")
        self.logger.info("=" * 60)
    ...
    def build_complete(self, output_dir: Optional[Path] = None, visualize: bool = True) -> bool:
        """Complete workflow: connect, build, save, visualize"""
        try:
            self.logger.info("=" * 60)
            self.logger.info("STARTING FLY ZONE CONSTRUCTION")
            self.logger.info("=" * 60)
            
            # Connect to AirSim if requested
            ...
            if visualize:
                ...
                # Visualize 4 horizontal slices
                z_slices = [0, self.grid_dims[2]//4, self.grid_dims[2]//2, 3*self.grid_dims[2]//4]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Synthetic obstacles (fast, no AirSim needed)
  python map_generator.py
  
  # AirSim LiDAR scanning (requires AirSim running)
  python map_generator.py --use-airsim
  
  # Custom grid parameters
  python map_generator.py --origin 0 0 20 --size 30 30 40 --cell-size 0.25
  
  # High resolution scan
  python map_generator.py --use-airsim --cell-size 0.1 --lidar-range 100
        """
    )
