...
class ObservationSpace:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        assert self.total_dim == 40, f"Observation dim must be 40, got {self.total_dim}"
        ...
        self.logger.info(
            f"Components: pose={self.config.pose_dim}, "
    ...
    def _compute_indices(self) -> Dict[str, Tuple[int, int]]:
        ...
        indices["localization_error"] = (
            start,
            start + self.config.localization_error_dim,
        )
    ...
    def _extract_pose_observation(
        self, drone_state: Dict[str, Any], slam_data: Dict[str, Any]
    ) -> np.ndarray:
        """
    ...
    def _extract_velocity_observation(self, drone_state: Dict[str, Any]) -> np.ndarray:
        ...
        velocity_obs = np.clip(
            velocity_obs, self.config.velocity_bounds[0], self.config.velocity_bounds[1]
        )

        return velocity_obs

    def _extract_goal_observation(
        self, drone_state: Dict[str, Any], goal_position: Tuple[float, float, float]
    ) -> np.ndarray:
        """
    ...
    def _extract_occupancy_observation(
        self, sensor_data: Dict[str, Any], current_time: float
    ) -> np.ndarray:
        """
        ...
        if current_time - self.last_occupancy_update >= update_interval:
            ...
            if "occupancy_histogram" in sensor_data:
                ...
                if (
                    occupancy_data is not None
                    and len(occupancy_data) == self.config.occupancy_dim
                ):
                    ...
                    self.cached_occupancy = self._generate_synthetic_occupancy(
                        sensor_data
                    )
            else:
                # No occupancy data available
                self.cached_occupancy = np.zeros(
                    self.config.occupancy_dim, dtype=np.float32
                )
    ...
    def _extract_localization_observation(
        self, slam_data: Dict[str, Any]
    ) -> np.ndarray:
        """
    ...
    def _normalize_observation(self, observation: np.ndarray) -> np.ndarray:
        ...
        if self.obs_count == 0:
            ...
            self.obs_var = (
                self.obs_count * self.obs_var + delta * (observation - self.obs_mean)
            ) / (self.obs_count + 1)
        ...
        epsilon = 1e-8
        normalized_obs = (observation - self.obs_mean) / (
            np.sqrt(self.obs_var) + epsilon
        )
    ...
    def get_observation_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "normalization": {"method": "running_mean_std", "clip_range": [-5.0, 5.0]},
        }
    ...
    def validate_observation(self, observation: np.ndarray) -> bool:
        ...
        if len(observation) != self.total_dim:
            self.logger.error(
                f"Invalid observation dimension: {len(observation)} != {self.total_dim}"
            )
            return False
