...
class AirSimEnvironment(gym.Env):
    ...
    def reset(
        ...
        try:
            ...
            self.logger.info(
                f"Drone has spawned at its actual location: {actual_spawn_location}"
            )
        except Exception as e:
            self.logger.error(
    ...
    def _log_initial_state(self) -> None:
        ...
        spawn_orientation = tuple(
            round(v, 3) for v in self.airsim_bridge.spawn_orientation
        )
    ...
    def step(
        ...
        if not self._logged_step_obs:
            self.logger.info(
                f"Observation vector shape during step: {observation.shape}"
            )
