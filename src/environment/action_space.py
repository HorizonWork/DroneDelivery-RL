...
class ActionConfig:
    ...
    action_low: Tuple[float, ...] = (
        ...
        -5.0,
        -1.0,
    )  # [vx_min, vy_min, vz_min, ω_min]
    action_high: Tuple[float, ...] = (
        5.0,
        5.0,
        5.0,
        1.0,
    )  # [vx_max, vy_max, vz_max, ω_max]
...
class ActionSpace:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.action_map = {
            "forward": np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32),  # vx
            "backward": np.array([-1.0, 0.0, 0.0, 0.0], dtype=np.float32),  # -vx
            "left": np.array([0.0, -1.0, 0.0, 0.0], dtype=np.float32),  # -vy
            "right": np.array([0.0, 1.0, 0.0, 0.0], dtype=np.float32),  # +vy
            "up": np.array([0.0, 0.0, 1.0, 0.0], dtype=np.float32),  # +vz
            "down": np.array([0.0, 0.0, -1.0, 0.0], dtype=np.float32),  # -vz
            "rotate_left": np.array([0.0, 0.0, 0.0, -1.0], dtype=np.float32),  # -yaw
            "rotate_right": np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32),  # +yaw
            "hover": np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float32),  # stop
        }
    ...
    def _apply_contextual_constraints(
        ...
        if (
            ...
            constrained_action[2] = max(
                constrained_action[2], -0.5
            )  # Limit descent rate
