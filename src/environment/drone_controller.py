...
class DroneController:
    ...
    def __init__(self, config: Dict[str, Any] = None, airsim_bridge: Any = None):
        ...
        self.logger.info(
            f"Safety features: collision_avoidance={self.collision_avoidance}"
        )
    ...
    def _validate_action(self, action: np.ndarray) -> bool:
        ...
        # Check bounds
        if (
            np.any(np.abs(action[:3]) > self.max_velocity)
            or np.abs(action[3]) > self.max_yaw_rate
        ):
            return False
    ...
    def _apply_safety_constraints(self, action: np.ndarray) -> np.ndarray:
        ...
        if self.last_drone_status:
            ...
            if (
                current_altitude < self.min_altitude and safe_action[2] > 0
            ):  # Descending (positive Z in NED)
                safe_action[2] = min(safe_action[2], -0.1)  # Force slight ascent

            # Prevent ascent if too high
            if (
                current_altitude > self.max_altitude and safe_action[2] < 0
            ):  # Ascending (negative Z in NED)
