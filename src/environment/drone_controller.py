...
class DroneController:
    ...
    def __init__(self, config: Dict[str, Any] = None, airsim_bridge: Any = None):
        ...
        # Initialize controller_state early to fix __del__ error
        self.controller_state = ControllerState()
        
        # Control parameters
        self.control_frequency = self.config.get("control_frequency", 20.0)  # Hz
        self.max_velocity = self.config.get("max_velocity", 5.0)  # m/s
        self.max_yaw_rate = self.config.get("max_yaw_rate", 1.0)  # rad/s
        
        # ✅ FIX: Add action scaling for PPO normalized actions
        self.action_scale = self.config.get("action_scale", 5.0)  # Scale from [-1,1] to physical
        self.expect_normalized_actions = self.config.get("expect_normalized_actions", True)
        
        # Safety parameters
        self.min_altitude = self.config.get("min_altitude", 0.5)  # meters
        self.max_altitude = self.config.get("max_altitude", 20.0)  # meters
        ...
        # ✅ FIX: More realistic limits for delivery drones
        self.max_acceleration = self.config.get("max_acceleration", 20.0)  # m/s² (2g)
        self.max_jerk = self.config.get("max_jerk", 50.0)  # m/s³
        
        # ✅ FIX: Make acceleration check optional
    ...
    def execute_action(self, action: np.ndarray):
        ...
        Args:
            action: 4D action vector [vx, vy, vz, yaw_rate]
                   Can be normalized [-1, 1] or physical units
        """
        # ✅ FIX: Scale action if normalized
        if self.expect_normalized_actions:
            # Scale from [-1, 1] to physical velocities
            scaled_action = action.copy()
            scaled_action[:3] = action[:3] * self.action_scale  # Velocity scaling
            scaled_action[3] = action[3] * self.max_yaw_rate  # Yaw rate scaling
        else:
            scaled_action = action.copy()
        
        if not self._validate_action(scaled_action):
            self.logger.warning("Invalid action received - using safe fallback")
            scaled_action = np.zeros(4)  # Hover command
    ...
    def _validate_action(self, action: np.ndarray) -> bool:
        ...
        Args:
            action: Action to validate (physical units)
        ...
        # Check velocity bounds (clip instead of reject)
        ...
        # ✅ FIX: Check acceleration limits (warning only unless strict mode)
        if self.controller_state.command_history:
            ...
            if accel_magnitude > self.max_acceleration:
                if self.strict_acceleration_check:
                    ...
                    # Just log as debug, allow action
                    self.logger.debug(
    ...
    def _apply_safety_constraints(self, action: np.ndarray) -> np.ndarray:
        ...
        Args:
            action: Raw action (physical units)
        ...
        # ✅ FIX: Clip to actual velocity limits
