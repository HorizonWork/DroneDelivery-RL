"""
IMU Preintegration
Implements IMU preintegration for tightly-coupled VI-SLAM.
"""
...
class PreintegrationResult:
    ...
    delta_rotation: np.ndarray  # rotation matrix
...
class IMUPreintegrator:
    """
    IMU preintegration for VI-SLAM.
    Implements preintegration theory from ORB-SLAM3 paper.
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # IMU parameters (matching AirSim and report specs)
        self.imu_frequency = config.get('imu_frequency', 200.0)         # Hz
        self.gravity = config.get('gravity', np.array([0, 0, 9.81]))    # m/s²
        
        # Noise parameters
        self.accel_noise_std = config.get('accel_noise_std', 0.02)      # m/s²/√Hz
        self.gyro_noise_std = config.get('gyro_noise_std', 0.0015)      # rad/s/√Hz
        self.accel_walk_std = config.get('accel_walk_std', 0.0002)      # m/s²√Hz
        self.gyro_walk_std = config.get('gyro_walk_std', 0.0001)        # rad/s√Hz
        
        # Bias estimation
        self.bias = IMUBias()
        self.estimate_bias = config.get('estimate_bias', True)
        
        # Preintegration storage
        self.measurements: deque = deque(maxlen=1000)  # Store recent measurements
        ...
        self.accel_cov = np.eye(3) * (self.accel_noise_std ** 2 / dt)
        self.gyro_cov = np.eye(3) * (self.gyro_noise_std ** 2 / dt)
        
        self.logger.info("IMU Preintegrator initialized")
        self.logger.info(f"Frequency: {self.imu_frequency}Hz")
        self.logger.info(f"Noise - Accel: {self.accel_noise_std}, Gyro: {self.gyro_noise_std}")
    ...
    def get_preintegration(self, end_timestamp: float, 
                          start_timestamp: Optional[float] = None) -> Optional[PreintegrationResult]:
        """
        Get IMU preintegration between timestamps.
        
        Args:
            end_timestamp: End time for preintegration
            start_timestamp: Start time (uses last keyframe if None)
            
        Returns:
            Preintegration result or None
        """
        # Check cache first
        ...
        for measurement in self.measurements:
            ...
            if start_timestamp is None:
                # Use all measurements up to end_timestamp
                if meas_time <= end_timestamp:
                    relevant_measurements.append(measurement)
            else:
                # Use measurements in time window
    ...
    def _preintegrate_measurements(self, measurements: List[Dict[str, Any]]) -> PreintegrationResult:
        """
        ...
        covariance = np.zeros((9, 9))  # [position, velocity, rotation]
        ...
        for i, measurement in enumerate(measurements[:-1]):
            ...
            # Preintegration (simplified implementation)
            # Real implementation would use proper manifold integration
            
            # Rotation integration (using Rodriguez formula approximation)
            ...
            delta_position += delta_velocity * dt + 0.5 * accel_world * dt ** 2
            
            # Update covariance (simplified)
            # Real implementation would use proper error propagation
            accel_var = np.trace(Q_accel) * dt ** 2
            gyro_var = np.trace(Q_gyro) * dt ** 2
            
            covariance[0:3, 0:3] += np.eye(3) * accel_var  # Position
            covariance[3:6, 3:6] += np.eye(3) * accel_var  # Velocity
            covariance[6:9, 6:9] += np.eye(3) * gyro_var   # Rotation
    ...
    def _update_bias_estimates(self):
        """Update IMU bias estimates using recent measurements."""
        if not self.estimate_bias or len(self.measurements) < 100:
            return
        
        try:
            # Simple bias estimation using measurement statistics
            recent_measurements = list(self.measurements)[-100:]
            
            # Calculate mean accelerations and gyroscope readings
            ...
            # Real implementation would use proper bias estimation
            
            # Accelerometer bias (should read gravity when stationary)
            ...
            if abs(gravity_error) > 0.5:  # Significant bias
                bias_update = actual_gravity * 0.01  # Small update
                self.bias.accel_bias = 0.9 * self.bias.accel_bias + 0.1 * bias_update
            
            # Gyroscope bias (should read zero when stationary)
            mean_gyro = np.mean(gyros, axis=0)
            if np.linalg.norm(mean_gyro) > 0.01:  # Significant bias
                bias_update = mean_gyro * 0.01  # Small update
                self.bias.gyro_bias = 0.9 * self.bias.gyro_bias + 0.1 * bias_update
            
            self.logger.debug(f"Bias updated - Accel: {self.bias.accel_bias}, Gyro: {self.bias.gyro_bias}")
    ...
    def reset(self):
        ...
        self.bias = IMUBias()  # Reset to default bias
        self.last_timestamp = 0.0
