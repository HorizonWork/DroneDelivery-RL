...
class TrajectoryMetrics:
    ...
    waypoint_tracking_error: float
    floor_transition_efficiency: float
    obstacle_avoidance_quality: float
...
class TrajectoryAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.smoothness_window = config.get(
            "smoothness_window", 5
        )
        self.velocity_smoothing = config.get(
            "velocity_smoothing", 0.1
        )
        self.curvature_threshold = config.get(
            "curvature_threshold", 0.5
        )

        self.altitude_change_threshold = config.get("altitude_threshold", 0.5)
        self.velocity_threshold = config.get("velocity_threshold", 0.3)
        self.maneuver_acceleration_threshold = config.get(
            "maneuver_threshold", 1.0
        )

        self.efficiency_benchmarks = {
            "min_path_efficiency": 0.80,
            "max_curvature": 0.5,
            "max_jerk": 3.0,
            "tracking_error_threshold": 0.5,
        }

        self.building_dims = config.get(
    ...
    def _analyze_single_trajectory(self, episode) - TrajectoryMetrics:
        ...
        acceleration_metrics = self._analyze_acceleration_profile(
        ...
        obstacle_avoidance_quality = 1.0 - min(
            1.0, getattr(episode, "clearance_violations", 0)  0.1
        )
    ...
    def _calculate_smoothness(
        self, trajectory: np.ndarray, timestamps: np.ndarray
    ) - float:
        ...
        for i in range(1, len(trajectory) - 1):
            ...
            if len(v1) == 3:
                ...
                if v1_magnitude  1e-6:
                    curvature = cross_magnitude / (v1_magnitude3)
        ...
        if curvatures:
            return {
                ...
                "curvature_violations": int(
                    np.sum(np.array(curvatures)  self.curvature_threshold)
                ),
            }
    ...
    def _analyze_acceleration_profile(
        self, trajectory: np.ndarray, timestamps: np.ndarray
    ) - Dict[str, float]:
        ...
        if len(velocities)  1:
            ...
            return {
                ...
                "high_acceleration_events": int(
                    np.sum(
                        acceleration_magnitudes  self.maneuver_acceleration_threshold
                    )
                ),
            }
    ...
    def _analyze_flight_phases(
        self, episodes_with_trajectories: List
    ) - Dict[str, Any]:
        ...
        for phase_name, phase_list in phase_data.items():
            if phase_list:
                phase_analysis[phase_name] = {
                    ...
                    "mean_energy": float(
                        np.mean([p.energy_consumption for p in phase_list])
                    ),
                    "mean_velocity": float(
                        np.mean([p.average_velocity for p in phase_list])
                    ),
                    "mean_smoothness": float(
                        np.mean([p.smoothness for p in phase_list])
                    ),
                }
    ...
    def _detect_flight_phases(
        self, trajectory: np.ndarray, total_time: float
    ) - List[FlightPhase]:
        ...
        if takeoff_end  0:
            ...
            takeoff_phase = FlightPhase(
                ...
                average_velocity=(
                    float(np.mean(velocity_magnitudes[:takeoff_end]))
                    if takeoff_end  0
                    else 0.0
                ),
                max_acceleration=0.0,
                smoothness=self._calculate_smoothness(
                    takeoff_trajectory, np.arange(len(takeoff_trajectory))  dt
                ),
            )
        ...
        for start_idx, end_idx in cruise_segments:
            ...
            cruise_phase = FlightPhase(
                ...
                energy_consumption=0.0,
                average_velocity=(
                    float(np.mean(velocity_magnitudes[start_idx:end_idx]))
                    if end_idx  start_idx
                    else 0.0
                ),
                max_acceleration=0.0,
                smoothness=self._calculate_smoothness(
                    cruise_trajectory, np.arange(len(cruise_trajectory))  dt
                ),
            )
    ...
    def _find_takeoff_end(self, altitudes: np.ndarray) - int:
        ...
        return len(altitudes)

    def _find_cruise_segments(
        self, trajectory: np.ndarray, velocities: np.ndarray
    ) - List[Tuple[int, int]]:
        ...
        for i in range(len(trajectory) - window_size):
            ...
            window_velocities = (
                velocities[i : i + window_size]
                if i + window_size = len(velocities)
                else []
            )
            ...
            if altitude_stable and velocity_stable:
                ...
                for j in range(
                    i + window_size, min(len(trajectory), i + window_size + 10)
                ):
                    if (
                        j  len(velocities)
                        and abs(altitudes[j] - np.mean(window_altitudes))
                         self.altitude_change_threshold
                        and abs(velocities[j] - np.mean(window_velocities))
    ...
    def _aggregate_trajectory_analysis(
        ...
        max_curvatures = [
            tm.curvature_metrics.get("max_curvature", 0) for tm in trajectory_metrics
        ]
        mean_curvatures = [
            tm.curvature_metrics.get("mean_curvature", 0) for tm in trajectory_metrics
        ]

        mean_velocities = [
            tm.velocity_metrics.get("mean_velocity", 0) for tm in trajectory_metrics
        ]
        max_velocities = [
            tm.velocity_metrics.get("max_velocity", 0) for tm in trajectory_metrics
        ]
        ...
        aggregated = {
            "path_efficiency": {
                ...
                "meets_benchmark": np.mean(path_efficiencies)
                = self.efficiency_benchmarks["min_path_efficiency"],
            },
            "smoothness": {
                "mean_score": float(np.mean(smoothness_scores)),
                "std_score": float(np.std(smoothness_scores)),
                "smooth_trajectories_percent": float(
                    np.mean(np.array(smoothness_scores)  0.8)  100
                ),
            },
            "curvature": {
                ...
                "curvature_violations": int(
                    np.sum(
                        np.array(max_curvatures)
                         self.efficiency_benchmarks["max_curvature"]
                    )
                ),
            },
            "velocity": {
                ...
                - float(np.std(mean_velocities) / (np.mean(mean_velocities) + 1e-6)),
            },
        }

        return aggregated

    def _analyze_floor_navigation(
        self, episodes_with_trajectories: List
    ) - Dict[str, Any]:
        ...
        analysis = {
            ...
            "episodes_with_transitions": len(
                [
                    1
                    for ep in episodes_with_trajectories
                    if self._has_floor_transitions(np.array(ep.trajectory))
                ]
            ),
            "average_transitions_per_episode": floor_data["total_floor_transitions"]
            / len(episodes_with_trajectories),
    ...
    def _has_floor_transitions(self, trajectory: np.ndarray) - bool:
        ...
        return max_altitude_change  2.5

    def _assess_trajectory_quality(
        self, trajectory_metrics: List[TrajectoryMetrics]
    ) - Dict[str, Any]:
        ...
        for metrics in trajectory_metrics:
            path_eff_score = min(
                1.0, metrics.path_efficiency / 0.8
            )
            smoothness_score = metrics.smoothness_score
            curvature_score = 1.0 - min(
                1.0, metrics.curvature_metrics.get("max_curvature", 0) / 1.0
            )

            quality_score = (
                0.4  path_eff_score + 0.4  smoothness_score + 0.2  curvature_score
            )
        ...
        quality_levels = {
            ...
            "good": np.sum(
                (np.array(quality_scores)  0.70)  (np.array(quality_scores) = 0.85)
            ),
            "fair": np.sum(
                (np.array(quality_scores)  0.50)  (np.array(quality_scores) = 0.70)
            ),
        ...
        return {
            ...
            "trajectories_above_threshold": int(
                np.sum(np.array(quality_scores)  0.70)
            ),
            "quality_consistency": 1.0 - float(np.std(quality_scores)),
            "improvement_potential": max(
                0.0, 0.85 - mean_quality
            ),
        }

    def export_trajectory_analysis(
        self, analysis: Dict[str, Any], filename: str = "trajectory_analysis.json"
    ):
    ...
    def get_trajectory_summary(self, analysis: Dict[str, Any]) - str:
        ...
        if "path_efficiency" in analysis:
            ...
            summary_lines.append(
                f"Path Efficiency: {eff.get('mean', 0):.3f}  {eff.get('std', 0):.3f}"
            )
            summary_lines.append(
                f"Efficiency Target: {' MET' if eff.get('meets_benchmark', False) else ' NOT MET'}"
            )
        ...
        if "smoothness" in analysis:
            ...
            summary_lines.append(
                f"Smooth Trajectories: {smooth.get('smooth_trajectories_percent', 0):.1f}"
            )

        if "quality_assessment" in analysis:
            quality = analysis["quality_assessment"]
            summary_lines.append(
                f"Overall Quality: {quality.get('overall_quality_score', 0):.3f}/1.0"
            )

            dist = quality.get("quality_distribution", {})
            summary_lines.append(
                f"Quality Distribution - Excellent: {dist.get('excellent', 0)}, "
