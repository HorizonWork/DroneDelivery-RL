...
class BaselineComparator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.baseline_configs = config.get(
            "baselines",
            {
                "A*_Only": {
    ...
    def _load_baseline_data(self):
        ...
        baseline_data_file = self.config.get(
            "baseline_data_file", "data/baselines/baseline_results.json"
        )
    ...
    def compare_with_baselines(
        self, rl_results: Dict[str, Any], method_name: str = "PPO_RL"
    ...
    def _statistical_comparison(
        ...
        metric_comparisons = [
            (
                "success_rate",
                rl_results.get("success_rate", 0),
                baseline.success_rate,
                "higher_better",
            ),
            (
                "mean_energy",
                rl_results.get("mean_energy", 0),
                baseline.mean_energy,
                "lower_better",
            ),
            (
                "mean_time",
                rl_results.get("mean_time", 0),
                baseline.mean_time,
                "lower_better",
            ),
            (
                "collision_rate",
                rl_results.get("collision_rate", 0),
                baseline.collision_rate,
                "lower_better",
            ),
            (
                "mean_ate",
                rl_results.get("mean_ate", 0),
                baseline.mean_ate,
                "lower_better",
            ),
        ]
        ...
        for metric_name, rl_value, baseline_value, direction in metric_comparisons:
            ...
            effect_size = (
                abs(rl_value - baseline_value)
                / max(
                    rl_results.get(
                        f'std_{metric_name.split("_")[1]}', baseline_value * 0.2
                    ),
                    baseline_value * 0.1,
                )
                if metric_name.startswith("mean_")
                else baseline_value * 0.1
            )
    ...
    def _log_comparison_summary(
        self, comparisons: Dict[str, List[ComparisonResult]], rl_method: str
    ):
            ...
            for comp in comparison_list:
                significance = (
                    ...
                    if comp.p_value < 0.001
                        ...
                        else "*" if comp.p_value < 0.05 else ""
                    )
                )

                self.logger.info(
                    f"  {comp.metric_name}: {comp.improvement_percent:+.1f}% "
    ...
    def calculate_energy_efficiency_gain(
        self, rl_energy: float, baseline_method: str = "A*_Only"
    ) -> float:
        """
    ...
    def validate_improvements(
        self, rl_results: Dict[str, Any], target_improvements: Dict[str, float] = None
    ) -> Dict[str, bool]:
        """
        ...
        if "A*_Only" in self.baseline_results:
            ...
            validation["energy_efficiency"] = (
                energy_gain >= target_improvements["energy_efficiency"]
            )

            # Success rate
            validation["success_rate"] = (
                rl_results.get("success_rate", 0) >= target_improvements["success_rate"]
            )

            # Collision reduction
            collision_reduction = (
                (
                    (baseline.collision_rate - rl_results.get("collision_rate", 0))
                    / baseline.collision_rate
                )
                * 100
            ...
            validation["collision_reduction"] = (
                collision_reduction >= target_improvements["collision_reduction"]
            )

            # Time improvement
            time_improvement = (
                (
                    (baseline.mean_time - rl_results.get("mean_time", 0))
                    / baseline.mean_time
                )
                * 100
            ...
            validation["time_improvement"] = (
                time_improvement >= target_improvements["time_improvement"]
            )
    ...
    def generate_statistical_report(
        ...
        for baseline_name, baseline_result in self.baseline_results.items():
            ...
            for comp in comparisons:
                ...
                report_lines.append(
                    f"    p-value: {comp.p_value:.4f}, Effect size: {comp.effect_size:.2f}"
                )
