...
class EnergyAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.thrust_efficiency = config.get(
        ...
        self.power_model = config.get(
            "power_model", "quadratic"
        )
        ...
        self.energy_weights = config.get(
            "energy_weights",
            {
                "thrust": 0.70,
                "avionics": 0.20,
        ...
        self.logger.info(
            f"Base power: {self.base_power}W, Drone mass: {self.drone_mass}kg"
        )
        self.logger.info(f"Power model: {self.power_model}")
        self.logger.info(
            f"Energy efficiency target: {self.efficiency_targets['energy_per_meter']}J/m"
        )
    ...
    def analyze_episodes(self, episode_results: List) - Dict[str, Any]:
        ...
        successful_episodes = [
            ep for ep in episode_results if getattr(ep, "success", False)
        ]
        ...
        self.logger.info(
            f"Analyzing energy consumption for {len(successful_episodes)} successful episodes"
        )
        ...
        analysis = {
            ...
            "energy_efficiency": {
                "mean_energy_per_meter": (
                    float(np.mean(energy_per_meter)) if energy_per_meter else 0.0
                ),
                "std_energy_per_meter": (
                    float(np.std(energy_per_meter)) if energy_per_meter else 0.0
                ),
                "meets_efficiency_target": (
                    np.mean(energy_per_meter)
                    = self.efficiency_targets["energy_per_meter"]
                    if energy_per_meter
                    else False
                ),
            },
            "power_analysis": {
                ...
                "meets_power_target": (
                    np.mean(average_power) = self.efficiency_targets["average_power"]
                    if average_power
                    else False
                ),
            },
        ...
        episodes_with_trajectories = [
            ...
            if hasattr(ep, "trajectory") and ep.trajectory
        ]

        if episodes_with_trajectories:
            advanced_analysis = self._advanced_energy_analysis(
                episodes_with_trajectories
            )
    ...
    def _advanced_energy_analysis(
        self, episodes_with_trajectories: List
    ) - Dict[str, Any]:

        flight_phases = {"takeoff": [], "cruise": [], "maneuvering": [], "landing": []}
        ...
        for episode in episodes_with_trajectories:
            ...
            takeoff_energy = self._estimate_phase_energy(
                trajectory[takeoff_mask], episode.flight_time / len(trajectory)
            )

            altitude_std = np.std(altitudes)
            cruise_mask = np.abs(altitudes - np.mean(altitudes))  altitude_std  0.5
            cruise_energy = self._estimate_phase_energy(
                trajectory[cruise_mask], episode.flight_time / len(trajectory)
            )
    ...
    def _estimate_phase_energy(
        self, trajectory_segment: np.ndarray, time_per_step: float
    ) - float:
        ...
        power_samples = self.base_power + k_factor  (velocity_magnitudes2)
    ...
    def _generate_efficiency_recommendations(
        ...
        recommendations.append(
            "Consider implementing regenerative braking during descents."
        )
        recommendations.append(
            "Optimize payload weight distribution for better efficiency."
        )

        return recommendations

    def calculate_energy_savings(
        self, rl_energy: float, baseline_energy: float
    ) - Dict[str, float]:
        ...
        annual_energy_savings = (
            absolute_savings  missions_per_day  days_per_year
        )
    ...
    def analyze_power_profile(
        ...
        for i in range(len(timestamps) - 1):
            ...
            acceleration = (
                acceleration_magnitudes[i] if i  len(acceleration_magnitudes) else 0.0
            )

            if self.power_model == "quadratic":
                thrust_power = self.base_power + 15.0  (velocity2)
            elif self.power_model == "cubic":
                thrust_power = self.base_power + 10.0  velocity + 8.0  (velocity3)
            else:
                thrust_power = self.base_power + 20.0  velocity

            maneuver_power = 5.0  (acceleration2)
    ...
    def compare_energy_efficiency(
        self, method_results: Dict[str, List]
    ) - Dict[str, Any]:
        ...
        for method_name, episodes in method_results.items():
            ...
            if successful:
                ...
                comparison[method_name] = {
                    "mean_energy": float(np.mean(energies)),
                    "energy_per_meter": (
                        float(np.mean(energy_per_meter)) if energy_per_meter else 0
                    ),
                    "average_power": (
                        float(np.mean(average_power)) if average_power else 0
                    ),
                    "energy_efficiency_score": self._calculate_efficiency_score(
                        energies, distances, times
                    ),
                }
        ...
        if len(comparison)  1:
            sorted_methods = sorted(
                comparison.items(),
                key=lambda x: x[1]["energy_efficiency_score"],
                reverse=True,
            )
    ...
    def _calculate_efficiency_score(
        self, energies: List[float], distances: List[float], times: List[float]
    ) - float:
        ...
        efficiency_score = (
            (mean_distance / mean_energy)  (1.0 / mean_time)  1000
        )
    ...
    def generate_energy_report(self, analysis: Dict[str, Any]) - str:
        ...
        report_lines.append(
            f"  Range: {energy_stats.get('min_energy', 0):.0f} - {energy_stats.get('max_energy', 0):.0f} J"
        )
        report_lines.append("")
        ...
        report_lines.append(
            f"  Energy per meter: {efficiency_stats.get('mean_energy_per_meter', 0):.1f} J/m"
        )
        report_lines.append(
            f"  Target (100 J/m): {' MET' if efficiency_stats.get('meets_efficiency_target', False) else ' NOT MET'}"
        )
        report_lines.append("")
        ...
        report_lines.append(
            f"  Average power: {power_stats.get('mean_power', 0):.1f} W"
        )
        report_lines.append(
            f"  Target (120 W): {' MET' if power_stats.get('meets_power_target', False) else ' NOT MET'}"
        )
        report_lines.append("")
        ...
        if breakdown:
            ...
            report_lines.append(
                f"  Thrust: {breakdown.get('thrust_energy', 0):.0f} J ({breakdown.get('thrust_percent', 0):.1f})"
            )
            report_lines.append(
                f"  Avionics: {breakdown.get('avionics_energy', 0):.0f} J ({breakdown.get('avionics_percent', 0):.1f})"
            )
            report_lines.append(
                f"  Communication: {breakdown.get('communication_energy', 0):.0f} J ({breakdown.get('communication_percent', 0):.1f})"
            )
            report_lines.append("")
    ...
    def export_energy_data(
        self, analysis: Dict[str, Any], filename: str = "energy_analysis.json"
    ):
