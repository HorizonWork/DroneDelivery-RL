...
class MetricSeries:
    ...
    def add_value(
        ...
        snapshot = MetricSnapshot(
            timestamp=timestamp or time.time(), value=value, metadata=metadata
        )
...
class MetricsCollector:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.metric_definitions = {
            "success_rate": MetricSeries(
                "success_rate", "", aggregation_method="mean"
            ),
            "energy_consumption": MetricSeries(
                "energy_consumption", "J", aggregation_method="sum"
            ),
            "flight_time": MetricSeries("flight_time", "s", aggregation_method="last"),
            "collision_occurred": MetricSeries(
                "collision_occurred", "bool", aggregation_method="max"
            ),
            "ate_error": MetricSeries("ate_error", "m", aggregation_method="mean"),
            "distance_to_goal": MetricSeries(
                "distance_to_goal", "m", aggregation_method="last"
            ),
            "path_efficiency": MetricSeries(
                "path_efficiency", "ratio", aggregation_method="mean"
            ),
            "velocity_magnitude": MetricSeries(
                "velocity_magnitude", "m/s", aggregation_method="mean"
            ),
            "acceleration_magnitude": MetricSeries(
                "acceleration_magnitude", "m/s²", aggregation_method="mean"
            ),
            "jerk_magnitude": MetricSeries(
                "jerk_magnitude", "m/s³", aggregation_method="mean"
            ),
            "control_smoothness": MetricSeries(
                "control_smoothness", "score", aggregation_method="mean"
            ),
            "num_replans": MetricSeries(
                "num_replans", "count", aggregation_method="sum"
            ),
            "clearance_violations": MetricSeries(
                "clearance_violations", "count", aggregation_method="sum"
            ),
            "thrust_energy": MetricSeries(
                "thrust_energy", "J", aggregation_method="sum"
            ),
            "avionics_energy": MetricSeries(
                "avionics_energy", "J", aggregation_method="sum"
            ),
            "power_consumption": MetricSeries(
                "power_consumption", "W", aggregation_method="mean"
            ),
            "waypoint_tracking_error": MetricSeries(
                "waypoint_tracking_error", "m", aggregation_method="mean"
            ),
            "path_deviation": MetricSeries(
                "path_deviation", "m", aggregation_method="mean"
            ),
            "planning_frequency": MetricSeries(
                "planning_frequency", "Hz", aggregation_method="mean"
            ),
        }
    ...
    def collect_step_metrics(self, step_data: Dict[str, Any]):
        ...
        if "position" in step_data and "goal_position" in step_data:
            ...
            self.current_episode_metrics["distance_to_goal"].add_value(
                distance, current_time
            )

        if "velocity" in step_data:
            vel_mag = np.linalg.norm(step_data["velocity"])
            self.current_episode_metrics["velocity_magnitude"].add_value(
                vel_mag, current_time
            )

        if "acceleration" in step_data:
            accel_mag = np.linalg.norm(step_data["acceleration"])
            self.current_episode_metrics["acceleration_magnitude"].add_value(
                accel_mag, current_time
            )
        ...
        if "min_clearance" in step_data:
            if step_data["min_clearance"]  0.5:
                self.current_episode_metrics["clearance_violations"].add_value(
                    1, current_time
                )
