"""
Phase 1 Trainer
Single floor + static obstacles training.
Foundation phase for basic navigation skills.
"""
...
class Phase1Trainer:
    """
    Phase 1: Single floor navigation with static obstacles.
    Target: 85% success rate, basic energy awareness.
    """

    def __init__(self, agent, environment, config: Dict[str, Any]):
        self.agent = agent
        self.environment = environment  # Single floor environment
        self.config = config
        self.logger = logging.getLogger(__name__)

        # Phase 1 specific parameters
        self.target_success_rate = 0.85  # 85% to advance
        self.max_episode_length = 1000  # Steps per episode
        self.obstacle_density = 0.1  # 10% occupancy

        # Phase 1 reward scaling
        self.reward_weights = {
            "goal_reached": 100.0,  # High reward for goal achievement
            "energy_penalty": -0.1,  # Light energy penalty to start
            "collision_penalty": -50.0,  # Strong collision avoidance
            "progress_reward": 1.0,  # Progress towards goal
            "time_penalty": -0.01,  # Slight time penalty
        }

        # Training tracking
        self.episode_results = []
        self.phase_timesteps = 0
        self.target_timesteps = 1_000_000  # 1M steps
    ...
    def train_phase(self) -> Dict[str, Any]:
        ...
        # Configure environment for Phase 1
        self._configure_phase1_environment()

        episode_count = 0
        recent_successes = deque(maxlen=50)  # Track recent success rate
        ...
        while self.phase_timesteps < self.target_timesteps:
            ...
            if episode_count % 100 == 0:
                current_success_rate = (
                    np.mean(recent_successes) if recent_successes else 0.0
                )
                self.logger.info(
                    f"Phase 1 Episode {episode_count}: "
                    f"Success rate: {current_success_rate*100:.1f}%, "
        ...
        final_success_rate = (
            np.mean(recent_successes) * 100 if recent_successes else 0.0
        )
    ...
    def _configure_phase1_environment(self):
        """Configure environment for Phase 1 training."""
        env_config = {
            "building_floors": 1,  # Single floor only
            "obstacle_density": self.obstacle_density,
            "dynamic_obstacles": False,  # Static obstacles only
            "max_episode_steps": self.max_episode_length,
            "reward_weights": self.reward_weights,
            "goal_spawn_distance": (2.0, 8.0),  # Start with closer goals
            "obstacle_complexity": "simple",  # Simple rectangular obstacles
        }
    ...
    def _run_phase1_episode(self) -> Dict[str, Any]:
        """Run single Phase 1 training episode."""
    ...
    def _evaluate_phase1(self) -> Dict[str, Any]:
        ...
        eval_results = []
        for _ in range(20):  # 20 episodes evaluation
            obs = self.environment.reset()
            episode_energy = 0.0
            done = False
    ...
    def set_starting_timestep(self, timestep: int):
        ...
        self.phase_timesteps = 0  # Reset phase counter
