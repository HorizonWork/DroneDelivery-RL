...
class Phase2Trainer:
    ...
    def train_phase(self) -> Dict[str, Any]:
        ...
        while self.phase_timesteps < self.target_timesteps:
            ...
            if episode_count % 100 == 0:
                ...
                self.logger.info(
                    f"Phase 2 Episode {episode_count}: "
            ...
            if (
                len(recent_successes) >= 50
                and np.mean(recent_successes) >= self.target_success_rate
                and sub_phase_progress > 0.5
            ):  # Must complete at least 50% of phase
        ...
        self.logger.info(
            f"Phase 2 completed: {final_success_rate:.1f}% success, {final_energy:.0f}J energy"
        )
    ...
    def _evaluate_phase2(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_floor_transitions": np.mean(
                [r["floor_transitions"] for r in eval_results]
            ),
            "dynamic_avoidance": np.mean(
                [r["dynamic_avoidances"] for r in eval_results if r["success"]]
            ),
