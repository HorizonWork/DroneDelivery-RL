...
class CurriculumTrainer:
    ...
    def _phase_completion_check(
        ...
        if criteria_met:
            self.logger.info(
                f"{phase.value} completion criteria met: {success_rate*100:.1f}% ≥ {threshold*100:.1f}%"
            )
        else:
            self.logger.warning(
                f"{phase.value} completion criteria NOT met: {success_rate*100:.1f}% < {threshold*100:.1f}%"
            )
    ...
    def _evaluate_curriculum_success(self) -> bool:
        ...
        targets_met = (
            ...
            and energy_efficiency >= 25.0  # 25% energy savings
        )
