...
class CheckpointManager:
    ...
    def save_checkpoint(
        ...
        self.logger.info(
            f"Performance: {success_rate:.1f}% success, {energy_efficiency:.0f}J energy"
        )
    ...
    def load_checkpoint(self, checkpoint_path: str, agent) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Restored to timestep {training_info.get('timestep', 0):,}, "
    ...
    def _update_best_models(self, checkpoint_info: CheckpointInfo, agent):
        ...
        # Best combined score
        combined_score = checkpoint_info.success_rate - (
            checkpoint_info.energy_efficiency / 1000.0
        )  # Normalize energy
    ...
    def _save_best_model(self, agent, model_type: str, checkpoint_info: CheckpointInfo):
        ...
        best_model_path = (
            self.best_models_dir / f"{self.experiment_name}_{model_type}.pt"
        )
        ...
        metadata_path = (
            self.best_models_dir / f"{self.experiment_name}_{model_type}_metadata.json"
        )
        with open(metadata_path, "w") as f:
            json.dump(metadata, f, indent=2)

        self.logger.info(
            f"New best {model_type} model saved: {checkpoint_info.success_rate:.1f}% success"
        )
    ...
    def _cleanup_old_checkpoints(self):
        ...
        for checkpoint_info in to_remove:
            try:
                ...
                self.logger.debug(
                    f"Removed old checkpoint: {Path(checkpoint_info.filepath).name}"
                )
            except Exception as e:
                self.logger.warning(
                    f"Failed to remove checkpoint {checkpoint_info.filepath}: {e}"
                )
    ...
    def get_best_checkpoint(
        ...
        if criteria == "success_rate":
            ...
            return min(
                self.checkpoint_registry,
                key=lambda x: (
                    x.energy_efficiency if x.energy_efficiency > 0 else float("inf")
                ),
            )
        else:  # combined
        ...
        return [
            {
                ...
                "age_hours": (
                    datetime.now()
                    - datetime.fromisoformat(info.created_at.replace("_", "T"))
                ).total_seconds()
                / 3600,
            }
            for info in sorted(
                self.checkpoint_registry, key=lambda x: x.timestep, reverse=True
            )
        ]
