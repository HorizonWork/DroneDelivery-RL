...
class TensorBoardLogger:
    ...
    def log_training_metrics(self, metrics: Dict[str, Any], timestep: int):
        ...
        if "episode_reward" in metrics:
            self.log_scalar(
                "Training/Episode_Reward", metrics["episode_reward"], timestep
            )

        if "episode_length" in metrics:
            self.log_scalar(
                "Training/Episode_Length", metrics["episode_length"], timestep
            )
        ...
        if "energy_consumption" in metrics:
            self.log_scalar(
                "Energy/Episode_Consumption", metrics["energy_consumption"], timestep
            )

        if "energy_efficiency" in metrics:
            self.log_scalar(
                "Energy/Efficiency_J_per_m", metrics["energy_efficiency"], timestep
            )
    ...
    def log_evaluation_results(self, eval_results: Dict[str, Any], timestep: int):
        ...
        self.log_scalar(
            "Evaluation/Success_Rate", eval_results.get("success_rate", 0), timestep
        )
        self.log_scalar(
            "Evaluation/Mean_Energy", eval_results.get("mean_energy", 0), timestep
        )
        self.log_scalar(
            "Evaluation/Mean_Time", eval_results.get("mean_time", 0), timestep
        )
        self.log_scalar(
            "Evaluation/Collision_Rate", eval_results.get("collision_rate", 0), timestep
        )
        self.log_scalar(
            "Evaluation/ATE_Error", eval_results.get("mean_ate", 0), timestep
        )
    ...
    def log_phase_transition(
        self, phase_name: str, phase_results: Dict[str, Any], timestep: int
    ):

        self.log_scalar(
            f"Curriculum/{phase_name}_Success_Rate",
            phase_results.get("final_success_rate", 0),
            timestep,
        )
        self.log_scalar(
            f"Curriculum/{phase_name}_Energy",
            phase_results.get("final_energy", 0),
            timestep,
        )
        self.log_scalar(
            f"Curriculum/{phase_name}_Training_Time",
            phase_results.get("training_time", 0),
            timestep,
        )
    ...
    def log_trajectory_analysis(
        self, trajectory_metrics: Dict[str, Any], timestep: int
    ):

        if "path_efficiency" in trajectory_metrics:
            eff = trajectory_metrics["path_efficiency"]
            self.log_scalar(
                "Trajectory/Path_Efficiency_Mean", eff.get("mean", 0), timestep
            )
            self.log_scalar(
                "Trajectory/Path_Efficiency_Std", eff.get("std", 0), timestep
            )

        if "smoothness" in trajectory_metrics:
            smooth = trajectory_metrics["smoothness"]
            self.log_scalar(
                "Trajectory/Smoothness_Score", smooth.get("mean_score", 0), timestep
            )

        if "quality_assessment" in trajectory_metrics:
            quality = trajectory_metrics["quality_assessment"]
            self.log_scalar(
                "Trajectory/Overall_Quality",
    ...
    def log_energy_analysis(self, energy_metrics: Dict[str, Any], timestep: int):
        ...
        if "energy_consumption" in energy_metrics:
            ...
            self.log_scalar(
                "Energy/Mean_Consumption", energy.get("mean_energy", 0), timestep
            )
            self.log_scalar(
                "Energy/Energy_Range", energy.get("energy_range", 0), timestep
            )

        if "energy_efficiency" in energy_metrics:
            eff = energy_metrics["energy_efficiency"]
            self.log_scalar(
                "Energy/Energy_Per_Meter", eff.get("mean_energy_per_meter", 0), timestep
            )
            self.log_scalar(
                "Energy/Efficiency_Target_Met",
                1.0 if eff.get("meets_efficiency_target", False) else 0.0,
                timestep,
            )
    ...
    def create_comparison_plot(
        ...
        if "A_Only" in baseline_results and "success_rate" in rl_results:
            baseline_success = baseline_results["A_Only"].get("success_rate", 0)
            rl_success = rl_results["success_rate"]
            improvement = (
                ((rl_success - baseline_success) / baseline_success)  100
                if baseline_success  0
                else 0
            )

            self.log_scalar(
                "Comparison/Success_Rate_Improvement_vs_AStar", improvement, timestep
            )

        if "A_Only" in baseline_results and "mean_energy" in rl_results:
            baseline_energy = baseline_results["A_Only"].get("mean_energy", 0)
            rl_energy = rl_results["mean_energy"]
            savings = (
                ((baseline_energy - rl_energy) / baseline_energy)  100
