...
class PPOAgent:
    ...
    def __init__(self, observation_dim: int, action_dim: int, config: Dict[str, Any]):
        ...
        ppo_config_dict = {
            ...
            "epochs_per_update": ppo_dict.get(
                "epochs_per_update", ppo_dict.get("epochs", 10)
            ),
            "discount_factor": ppo_dict.get(
                "discount_factor", ppo_dict.get("gamma", 0.99)
            ),
            "gae_lambda": ppo_dict.get("gae_lambda", 0.95),
            "clip_range": ppo_dict.get("clip_range", ppo_dict.get("clip_epsilon", 0.2)),
            "value_loss_coefficient": ppo_dict.get(
                "value_loss_coefficient", ppo_dict.get("value_loss_coef", 0.5)
            ),
            "entropy_coefficient": ppo_dict.get(
                "entropy_coefficient", ppo_dict.get("entropy_coef", 0.01)
            ),
        ...
        self.logger.info(
            f"Observation dim: {observation_dim}, Action dim: {action_dim}"
        )
        self.logger.info(
            f"Hyperparameters: lr={self.config.learning_rate}, "
    ...
    def update_policy(
        ...
        for epoch in range(self.config.epochs_per_update):
            epoch_metrics = self._ppo_epoch_update(
                observations, actions, old_log_probs, advantages_tensor, returns_tensor
            )
    ...
    def train_episode(self, episode_reward: float, episode_length: int):
        ...
        if len(self.episode_rewards) >= 10:  # Wait for some episodes
            ...
            self.learning_metrics["mean_episode_length"].append(
                np.mean(list(self.episode_lengths)[-10:])
            )
    ...
    def load_model(self, filepath: str):
        ...
        try:
            ...
            self.learning_metrics = save_data.get(
                "learning_metrics", self.learning_metrics
            )
    ...
    def get_training_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            "training_progress": {
                ...
                "episode_count": self.episode_count,
                "total_environment_steps": self.total_environment_steps,
                "steps_per_episode": self.total_environment_steps
                / max(1, self.episode_count),
            },
            "performance": {
                "mean_episode_reward": (
                    np.mean(list(self.episode_rewards)) if self.episode_rewards else 0.0
                ),
                "std_episode_reward": (
                    np.std(list(self.episode_rewards)) if self.episode_rewards else 0.0
                ),
                "mean_episode_length": (
                    np.mean(list(self.episode_lengths)) if self.episode_lengths else 0.0
                ),
                "recent_rewards": (
            ...
            "losses": {
                "recent_policy_loss": (
                    np.mean(list(self.training_losses["policy_loss"])[-10:])
                    if self.training_losses["policy_loss"]
                    else 0.0
                ),
                "recent_value_loss": (
                    np.mean(list(self.training_losses["value_loss"])[-10:])
                    if self.training_losses["value_loss"]
                    else 0.0
                ),
                "recent_entropy_loss": (
                    np.mean(list(self.training_losses["entropy_loss"])[-10:])
                    if self.training_losses["entropy_loss"]
                    else 0.0
                ),
            },
            "hyperparameters": {
