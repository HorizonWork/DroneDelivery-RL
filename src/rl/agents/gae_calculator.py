...
class GAECalculator:
    ...
    def __init__(self,
        ...
        """
        Multiple aliases are supported so configs/tests can pass either `lam`,
        `lambda_`, or `gae_lambda`, likewise `gamma` or `discount_factor`.
        """
        lambda_value = (
            gae_lambda if gae_lambda is not None else
            (lambda_ if lambda_ is not None else lam)
        )
