...
class PolicyNetwork(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Action space bounds
        self.action_bounds = config.get(
            "action_bounds",
            {"velocity": [-5.0, 5.0], "yaw_rate": [-2.0, 2.0]},  # m/s  # rad/s
        )
    ...
    def set_exploration_noise(self, noise_std: Union[float, List[float]]):
        ...
        self.logger.info(
            f"Exploration noise updated: {torch.exp(self.log_std).detach().cpu().numpy()}"
        )
...
class MultiActionPolicyNetwork(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.discrete_action_dims = config.get(
            "discrete_action_dims", []
        )  # List of discrete action space sizes
        ...
        if self.continuous_action_dim > 0:
            self.continuous_mean = nn.Linear(
                self.hidden_sizes[-1], self.continuous_action_dim
            )
            self.continuous_log_std = nn.Parameter(
                torch.zeros(self.continuous_action_dim)
            )
        ...
        self.logger = logging.getLogger(__name__)
        self.logger.info(
            f"Multi-Action Policy: {self.continuous_action_dim}D continuous + {len(self.discrete_action_dims)} discrete"
        )
    ...
    def forward(
    ...
    ) -> Tuple[Optional[Normal], List[Categorical]]:
        """
        ...
        if self.continuous_action_dim > 0:
            ...
            continuous_std = torch.exp(self.continuous_log_std).expand_as(
                continuous_mean
            )
