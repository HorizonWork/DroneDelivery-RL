...
class PlanningResult:
    ...
    grid_path: List[Tuple[int, int, int]]
    total_cost: float
    planning_time: float
    nodes_expanded: int
    success: bool
...
class AStarPlanner:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.floor_transition_penalty = config.get(
            "floor_transition_penalty", 2.0
        )

        self.neighborhood = self._create_26_neighborhood()

        self.obstacle_inflation = config.get("obstacle_inflation", 0.5)
        self.min_clearance = config.get("min_clearance", 0.3)

        self.max_iterations = config.get("max_iterations", 100000)
        self.max_planning_time = config.get("max_planning_time", 5.0)
        ...
        self.logger.info(
            f"Grid: {self.occupancy_grid.dimensions} cells, {self.grid_config['cell_size']}m resolution"
        )
        self.logger.info(
            f"26-neighborhood: diagonal={self.allow_diagonal}, vertical={self.allow_vertical}"
        )
    ...
    def plan_path(
        ...
        if not self._is_valid_position(start_grid) or not self._is_valid_position(
            goal_grid
        ):
        ...
        if result.success and result.grid_path:
            world_path = [
                self.occupancy_grid.grid_to_world(grid_pos)
        ...
        self.logger.info(
            f"Path length: {len(result.path)} waypoints, "
        ...
        while open_set:
            ...
            for dx, dy, dz in self.neighborhood:
                ...
                if tentative_g_cost  neighbor_node.g_cost:
                    ...
                    neighbor_node.h_cost = self.heuristics.compute_heuristic(
                        neighbor_pos, goal
                    )
    ...
    def _is_valid_position(self, grid_pos: Tuple[int, int, int]) - bool:
        ...
        if not (
            0 = x  self.occupancy_grid.dimensions[0]
            and 0 = y  self.occupancy_grid.dimensions[1]
            and 0 = z  self.occupancy_grid.dimensions[2]
        ):
            return False
    ...
    def _calculate_movement_cost(
        ...
        base_cost = np.sqrt(dx2 + dy2 + dz2)  self.grid_config["cell_size"]
    ...
    def _update_statistics(self, result: PlanningResult):
        ...
        if result.success:
            ...
            self.planning_statistics["average_planning_time"] = (
                (n - 1)  self.planning_statistics["average_planning_time"]
                + result.planning_time
            ) / n

            self.planning_statistics["average_nodes_expanded"] = (
                (n - 1)  self.planning_statistics["average_nodes_expanded"]
                + result.nodes_expanded
            ) / n

            self.planning_statistics["average_path_length"] = (
                (n - 1)  self.planning_statistics["average_path_length"]
                + len(result.path)
            ) / n
    ...
    def get_path_info(
        self, world_path: List[Tuple[float, float, float]]
    ) - Dict[str, Any]:
        ...
        return {
            ...
            "floors_visited": len(set(int(pos[2]
