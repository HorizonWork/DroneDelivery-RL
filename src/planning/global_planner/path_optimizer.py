...
class PathOptimizer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger.info(
            f"Weights - Smoothing: {self.smoothing_weight}, "
    ...
    def optimize_path(
        ...
        occupancy_grid: Optional[Any] = None,
        ...
        try:
            if self.method == "spline":
                ...
                optimized_path = self._optimize_with_gradient_descent(
                    path, occupancy_grid
                )
            elif self.method == "both":
                spline_path = self._optimize_with_spline(path)
                optimized_path = self._optimize_with_gradient_descent(
                    spline_path, occupancy_grid
                )
            else:
                optimized_path = path
    ...
    def _optimize_with_spline(
        ...
        try:
            ...
            distances = np.cumsum(
                np.concatenate(
                    ([0], np.linalg.norm(np.diff(path_array, axis=0), axis=1))
                )
            )
    ...
    def _optimize_with_gradient_descent(
        self, path: List[Tuple[float, float, float]], occupancy_grid: Optional[Any]
    ) - List[Tuple[float, float, float]]:
    ...
    def _calculate_path_cost(
        ...
        if len(path)  2:
            ...
            energy_cost = np.sum(velocity_magnitudes2) + np.sum(
                acceleration_magnitudes2
            )
    ...
    def analyze_path_quality(
        ...
        if len(path)  2:
            ...
            for i in range(len(unit_vectors) - 1):
                dot_product = np.clip(
                    np.dot(unit_vectors[i], unit_vectors[i + 1]), -1, 1
                )
        ...
        if len(path)  1:
            ...
            energy_estimate = np.sum(velocities2)
        else:
            energy_estimate = 0.0
        ...
        return {
            ...
            "smoothness_score": float(
                1.0 / (1.0 + mean_curvature)
            ),
        }

    def resample_path(
        self, path: List[Tuple[float, float, float]], target_spacing: float = 0.5
    ) - List[Tuple[float, float, float]]:
    ...
    def _optimize_with_gradient_descent(
        ...
        for iteration in range(5):
            for i in range(1, len(path_array) - 1):
                ...
                alpha = 0.3
                optimized_array[i] = (1 - alpha)  path_array[
                    i
    ...
    def compare_paths(
        ...
        optimized_path: List[Tuple[float, float, float]],
    ) - Dict[str, Any]:
        ...
        for key in ["total_length", "mean_curvature", "energy_estimate"]:
            if key in original_metrics and key in optimized_metrics:
                ...
                if original_val  0:
                    improvement_percent = (
                        (original_val - optimized_val) / original_val  100
                    )
