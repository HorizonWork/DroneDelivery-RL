...
class OccupancyGrid3D:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger.info(
            f"Grid dimensions: {self.dimensions} = {total_cells} total cells"
        )
        ...
        self.logger.info(
            f"Cell size: {self.cell_size}m, Inflation: {self.inflation_radius}m"
        )
    ...
    def _update_combined_grid(self):
        ...
        if self.inflation_cells > 0:
            combined = binary_dilation(
                combined, structure=self.inflation_kernel
            ).astype(np.int8)
    ...
    def get_min_clearance(
        self, grid_pos: Tuple[int, int, int], search_radius: int = 5
    ) -> float:
        """
        ...
        return (
            ...
            else search_radius * self.cell_size
        )
    ...
    def update_grid(self, new_grid: np.ndarray):
        ...
        if new_grid.shape == self.dimensions:
            ...
            self.logger.error(
                f"Grid shape mismatch: {new_grid.shape} vs {self.dimensions}"
            )
    ...
    def is_floor_accessible(self, floor: int, position: Tuple[float, float]) -> bool:
        ...
        floor_z = (
            floor - 1
        ) * floor_height_cells + floor_height_cells // 2  # Mid-floor

        grid_pos = (grid_x, grid_y, floor_z)
        return self.is_free(grid_pos)

    def find_path_to_floor(
        self, current_pos: Tuple[int, int, int], target_floor: int
