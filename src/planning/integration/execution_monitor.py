"""
Execution Monitor
Monitors path execution progress and triggers replanning when needed.
Ensures safe mission execution with real-time monitoring.
"""
...
class ExecutionMonitor:
    """
    Path execution monitoring system.
    ...
    def __init__(
        ...
        self.max_position_deviation = config.get("max_deviation", 2.0)  # meters

        # Replanning triggers
        self.obstacle_proximity_threshold = config.get(
            "obstacle_proximity", 1.5
        )  # meters
        self.blocked_path_threshold = config.get("blocked_threshold", 0.8)  # meters
        self.execution_timeout = config.get("execution_timeout", 300.0)  # seconds

        # Safety monitoring
        self.safety_check_frequency = config.get("safety_frequency", 5.0)  # Hz
        self.min_safety_clearance = config.get("min_safety_clearance", 0.5)  # meters
    ...
    def start_execution(self, path: List[Tuple[float, float, float]]):
        ...
        self._log_event(
            "EXECUTION_STARTED",
            "INFO",
            f"Started monitoring path with {len(path)} waypoints",
        )
    ...
    def _monitoring_loop(self):
        ...
        while self.is_monitoring:
            try:
                ...
                # Update current status (would get from environment)
                ...
                if current_position:
                    ...
                    # Check safety (at lower frequency)
                    if (
                        time.time() - last_safety_check
                        > 1.0 / self.safety_check_frequency
                    ):
        ...
        with self.monitor_lock:
            ...
            if len(self.position_history) > 1:
                ...
                distance_step = np.linalg.norm(
                    np.array(current_position) - np.array(prev_pos)
                )
                ...
                if elapsed_time > 0:
                    self.metrics.average_speed = (
                        self.metrics.distance_traveled / elapsed_time
                    )
    ...
    def _calculate_progress(self, current_position: Tuple[float, float, float]):
        """Calculate path execution progress."""
        ...
        if len(self.current_path) > 1:
            self.metrics.progress_percentage = (
                closest_waypoint_idx / (len(self.current_path) - 1)
            ) * 100
    ...
    def _check_progress(self, current_position: Tuple[float, float, float]):
        """Check execution progress and waypoint advancement."""
        ...
        if distance_to_waypoint <= self.waypoint_tolerance:
            ...
            if self.current_waypoint_index >= len(self.current_path):
                ...
                self._log_event(
                    "PATH_COMPLETED", "INFO", "Path execution completed successfully"
                )

        # Check for position deviation
            ...
            self._trigger_replan(
                ReplanTrigger.POSITION_DEVIATION,
                {
    ...
    def _check_safety(
        ...
        """Check safety constraints and clearances."""
        if not obstacles:
            return
        ...
        # Check minimum clearance violation
        if min_clearance < self.min_safety_clearance:
            ...
            self._trigger_replan(
                ReplanTrigger.SAFETY_VIOLATION,
                {
                    "current_position": current_position,
                    "min_clearance": min_clearance,
                    "obstacles": obstacles,
    ...
    def _check_replan_triggers(
        ...
        """Check for conditions that should trigger replanning."""
        current_time = time.time()
        ...
        if elapsed_time > self.execution_timeout:
            ...
            self._trigger_replan(
                ...
                {"elapsed_time": elapsed_time, "timeout_limit": self.execution_timeout},
            )
            return

        # Check obstacle proximity to upcoming path
        if self._check_upcoming_path_blocked(current_position, obstacles):
            self._log_event(
                "PATH_BLOCKED", "WARNING", "Upcoming path blocked by obstacles"
            )

            self._trigger_replan(
                ReplanTrigger.PATH_BLOCKED,
                {"current_position": current_position, "blocking_obstacles": obstacles},
            )

    def _check_upcoming_path_blocked(
        self,
        current_position: Tuple[float, float, float],
        obstacles: List[Tuple[float, float, float]],
    ) -> bool:
        """
        Check if upcoming path segment is blocked by obstacles.
        ...
        end_idx = min(
            self.current_waypoint_index + lookahead_waypoints, len(self.current_path)
        )
    ...
    def _get_current_obstacles(self) -> List[Tuple[float, float, float]]:
        ...
        try:
            ...
            if (
                hasattr(sensor_data, "occupancy_histogram")
                and sensor_data.occupancy_histogram is not None
            ):
            ...
            # Alternative: Get obstacles from depth image directly
                ...
                hasattr(sensor_data, "depth_features")
                and sensor_data.depth_features is not None
            ):
                depth_features = sensor_data.depth_features

                if (
                    "min_distance" in depth_features
                    and depth_features["min_distance"] < 2.0
                ):
    ...
    def update_path(self, new_path: List[Tuple[float, float, float]]):
        ...
        with self.monitor_lock:
            ...
            # Keep current waypoint index if still valid
        ...
        self._log_event(
            "PATH_UPDATED", "INFO", f"Path updated: {len(new_path)} waypoints"
        )

    def add_replan_callback(
        self, callback: Callable[[ReplanTrigger, Dict[str, Any]], None]
    ):
        ...
        with self.monitor_lock:
            ...
            return {
                ...
                "metrics": {
                    ...
                    "execution_time": self.metrics.current_time
                ...
                "events": {
                    ...
                    "recent_events": len(
                        [e for e in self.events if time.time() - e.timestamp < 60]
                    ),
                },
                "configuration": {
