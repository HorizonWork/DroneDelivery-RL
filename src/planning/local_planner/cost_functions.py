...
class SRRTCostFunction:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.curvature_window = config.get(
            "curvature_window", 3
        )

        self.logger.info("S-RRT Cost Function initialized")
        self.logger.info(
            f"Equation (3): C = ℓ + {self.params.lambda_c}(1/dmin)² + {self.params.lambda_kappa}κ²"
        )
        self.logger.info(f"Min clearance: {self.params.min_clearance}m")

    def calculate_edge_cost(
        self, parent_node, child_node, obstacles: List[Tuple[float, float, float]]
    ) - float:

        length_cost = np.linalg.norm(child_node.position - parent_node.position)

        clearance_cost = self._calculate_clearance_cost(
    ...
    def _calculate_clearance_cost(
        ...
        num_samples = max(
            3, int(np.linalg.norm(child_node.position - parent_node.position) / 0.2)
        )
        min_clearance = float("inf")

        for i in range(num_samples + 1):
            t = i / num_samples
            sample_point = parent_node.position + t  (
                child_node.position - parent_node.position
            )

            for obstacle_pos in obstacles:
                obstacle_array = np.array(obstacle_pos)
                distance = (
                    np.linalg.norm(sample_point - obstacle_array) - self.obstacle_radius
                )
                min_clearance = min(
                    min_clearance, max(0.01, distance)
                )
    ...
    def _calculate_curvature_cost(self, parent_node, child_node) - float:
        ...
        curvature = np.linalg.norm(second_derivative) / (total_length2)

        curvature_penalty = self.params.lambda_kappa  (curvature2)
    ...
    def evaluate_path_cost(
        ...
        for i in range(len(path) - 1):
            ...
            edge_clearance_cost = self._calculate_edge_clearance_cost(
                parent_pos, child_pos, obstacles
            )
            total_clearance_cost += edge_clearance_cost

            if i  0:
                grandparent_pos = path[i - 1]
                edge_curvature_cost = self._calculate_edge_curvature_cost(
                    grandparent_pos, parent_pos, child_pos
                )
    ...
    def _calculate_edge_clearance_cost(
        self,
    ...
    def _calculate_edge_curvature_cost(
        ...
        curvature = np.linalg.norm(second_derivative) / (total_length2)
        return self.params.lambda_kappa  (curvature2)
    ...
    def validate_cost_parameters(self) - Dict[str, bool]:
        ...
        validation = {
            ...
            "parameters_balanced": self.params.lambda_c / self.params.lambda_kappa
             10.0,
        }
