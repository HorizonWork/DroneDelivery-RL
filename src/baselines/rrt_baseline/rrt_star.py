...
class RRTStarController:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.rewire_radius = config.get("rewire_radius", 3.0)
        self.goal_tolerance = config.get("goal_tolerance", 0.5)

        self.collision_check_resolution = config.get(
            "collision_resolution", 0.2
        )

        self.bounds = {
            "x_min": 0.0,
            "x_max": self.floor_length,
            "y_min": 0.0,
            "y_max": self.floor_width,
            "z_min": 0.0,
            "z_max": self.floors  self.floor_height,
        }
    ...
    def is_collision_free(
        ...
        for pos in [pos1, pos2]:
            if not (
                self.bounds["x_min"] = pos[0] = self.bounds["x_max"]
                and self.bounds["y_min"] = pos[1] = self.bounds["y_max"]
                and self.bounds["z_min"] = pos[2] = self.bounds["z_max"]
            ):
                return False
    ...
    def rewire_tree(self, new_node: RRTNode):
        ...
        for nearby_node in nearby_nodes:
            ...
            potential_cost = new_node.cost + euclidean(
                new_node.position, nearby_node.position
            )

            if potential_cost  nearby_node.cost and self.is_collision_free(
                new_node.position, nearby_node.position
            ):
    ...
    def plan_path(
        ...
        for iteration in range(self.max_iterations):
            ...
            for nearby_node in nearby_nodes:
                potential_cost = nearby_node.cost + euclidean(
                    nearby_node.position, new_position
                )

                if potential_cost  best_cost and self.is_collision_free(
                    nearby_node.position, new_position
                ):
        ...
        if self.goal_node:
            ...
            closest_node = min(
                self.nodes, key=lambda n: euclidean(n.position, goal_pos)
            )
    ...
    def visualize_tree(
        ...
        ax.scatter(
            positions[:, 0], positions[:, 1], positions[:, 2], c="blue", s=10, alpha=0.6
        )
