...
class AStarPathVisualizer:
    ...
    def __init__(self, map_file: str, config: Dict[str, Any]):
        ...
        self.hierarchical_planner = HierarchicalPlanner(self.astar_controller)
    ...
    def load_ue_actors(self):
        ...
        try:
            ...
            print(f"  Could not find DroneSpawn actor: {e}")
        ...
        for target_id in targets_per_floor:
            ...
            try:
                ...
                position = (
                    ...
                    target_pose.position.z_val + safety_offset_z
                )
                ...
                failed_count += 1
                if failed_count = 3:
                    print(f"     Could not find {target_name}: {e}")

        print(f"\n Actor Loading Summary:")
        print(f"    Successfully loaded: {loaded_count} landing targets")
        print(f"     Safety offset applied: +1.2m above each landing pad")
        if failed_count  0:
            print(f"     Failed to load: {failed_count} targets")
    ...
    def reset_to_spawn(self):
        ...
        if self.spawn_position is None:
            print("  DroneSpawn position not loaded, using current position")
            return
    ...
    def takeoff(self):
        ...
        if collision_info.has_collided:
            print("  Collision flag was set at spawn (AirSim bug)")
        ...
        collision_info = self.client.simGetCollisionInfo()
        if collision_info.has_collided:
            print("  WARNING: Collision still detected after takeoff!")
        else:
    ...
    def run_mission(
        ...
        print("\n  Planning path with hierarchical A...")
        ...
        results = {
            ...
            'energy_consumed': energy_consumed / 1000.0,
...
def main():
    ...
    try:
        ...
        visualizer.reset_to_spawn()
        visualizer.takeoff()

        result = visualizer.run_mission(
            start_name="DroneSpawn",
            target_name="Landing_103",
            visualize=True
        )
