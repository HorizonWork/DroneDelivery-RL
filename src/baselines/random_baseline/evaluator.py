...
class RandomEvaluator:
    ...
    def evaluate_episode(self, random_agent, environment) - EpisodeResult:
        ...
        result.ate_error = (
            environment.get_ate_error()
            if hasattr(environment, "get_ate_error")
            else 0.0
        )
    ...
    def evaluate_multiple_episodes(
        self, random_agent, environment, num_episodes: int = None
    ) - EvaluationMetrics:
    ...
    def _compute_metrics(self) - EvaluationMetrics:
        ...
        metrics.target_success_rate = 5.0
        metrics.target_energy_mean = 2000.0
        metrics.target_time_mean = 280.0
        metrics.target_collision_rate = 15.0
        metrics.target_ate_mean = 0.20
    ...
    def print_results(self, metrics: EvaluationMetrics):
        ...
        print(
            f"Success rate: {metrics.success_rate:.1f} (expected: {metrics.target_success_rate:.1f})"
        )
        print(
            f"Collision rate: {metrics.collision_rate:.1f} (expected: {metrics.target_collision_rate:.1f})"
        )
