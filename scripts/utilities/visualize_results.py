...
class ResultsVisualizer:
    ...
    def visualize_training_results(
        self, training_results_file: str, output_dir: str
    ) - Dict[str, Any]:
        ...
        distribution_plots = self._plot_performance_distributions(
    ...
    def visualize_evaluation_results(
        self, evaluation_results_file: str, baseline_results_file: str, output_dir: str
    ) - Dict[str, Any]:
        ...
        energy_plots = self._plot_evaluation_energy_analysis(
            evaluation_data, output_path
        )
        plots_created.extend(energy_plots)
        ...
        self.logger.info(
            f"Evaluation visualizations created: {len(plots_created)} plots"
        )
        return viz_results

    def _plot_training_curves(
        self, training_data: Dict[str, Any], output_path: Path
    ) - List[str]:
        ...
        if episode_rewards:
            ...
            if len(episode_rewards)  50:
                ...
                axes[0, 0].plot(
                    moving_avg,
                    color=self.viz_config["color_scheme"]["accent"],
                    linewidth=2,
        ...
        if success_rates:
            axes[0, 1].plot(
                success_rates,
                color=self.viz_config["color_scheme"]["success"],
                linewidth=2,
            )
            axes[0, 1].axhline(
                y=96,
                color=self.viz_config["color_scheme"]["error"],
                linestyle="--",
                label="Target (96)",
            )
        ...
        if energy_consumptions:
            axes[1, 0].plot(
                energy_consumptions,
                color=self.viz_config["color_scheme"]["warning"],
                alpha=0.7,
            )
            ...
            if len(energy_consumptions)  50:
                window = 50
                moving_avg = [
                    np.mean(energy_consumptions[max(0, i - window) : i + 1])
                    for i in range(len(energy_consumptions))
                ]
                axes[1, 0].plot(
                    moving_avg,
                    color=self.viz_config["color_scheme"]["error"],
        ...
        if episode_rewards and energy_consumptions:
            norm_rewards = (
                np.array(episode_rewards) / max(episode_rewards)
                if episode_rewards
                else []
            )
            norm_energy = (
                1 - (np.array(energy_consumptions) / max(energy_consumptions))
                if energy_consumptions
                else []
            )
        ...
        plt.savefig(
            training_curves_file, dpi=self.viz_config["dpi"], bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_performance_comparison(
        ...
        bars2 = axes[0, 1].bar(
            methods, energies, yerr=energy_stds, color=colors, capsize=5
        )
        ...
        for method in methods:
            ...
            energy_score = max(
                0, 100 - (data["energy"] / 35)
            )
            time_score = max(0, 100 - (data["time"] / 1.2))
            collision_score = max(
                0, 100 - data["collisions"]  2.5
            )
            ate_score = max(0, 100 - (data["ate"] / 0.001)  1.25)
    ...
    def _plot_target_achievements(
        self, evaluation_data: Dict[str, Any], output_path: Path
    ) - List[str]:
        ...
        targets = {
            ...
            "ATE Error\n5cm": {
                "current": performance_metrics.get("mean_ate", 0)
                 100,
                "target": 5.0,
        ...
        bars_current = ax.bar(
            ...
            color=[
                (
                    self.viz_config["color_scheme"]["success"]
                    if met
                    else self.viz_config["color_scheme"]["warning"]
                )
        ...
        for i, (bar_current, bar_target, current, target, target_name) in enumerate(
            zip(bars_current, bars_target, current_values, target_values, target_names)
        ):
            height = bar_current.get_height()
            unit = targets[target_name]["unit"]
            ax.text(
                bar_current.get_x() + bar_current.get_width() / 2.0,
                height,
                f"{current:.1f}{unit}",
                ha="center",
                va="bottom",
            ...
            ax.text(
                bar_target.get_x() + bar_target.get_width() / 2.0,
                height,
                f"{target:.1f}{unit}",
                ha="center",
                va="bottom",
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize training and evaluation results"
    )
    parser.add_argument(
        "--training-results", type=str, help="Path to training results JSON"
    )
    parser.add_argument(
        "--evaluation-results", type=str, help="Path to evaluation results JSON"
    )
    parser.add_argument(
        "--baseline-results", type=str, help="Path to baseline results JSON"
    )
    parser.add_argument(
        "--output", type=str, default="results/visualizations", help="Output directory"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    if args.training_results:
        training_viz = visualizer.visualize_training_results(
            args.training_results, args.output
        )
        print(
            f" Training visualizations: {training_viz['plots_created']} plots created"
        )
    ...
    if args.evaluation_results:
        ...
        print(
            f" Evaluation visualizations: {evaluation_viz['plots_created']} plots created"
        )
