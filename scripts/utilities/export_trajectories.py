...
class TrajectoryExporter:
    ...
    def export_from_training_results(
        self, training_results_file: str, output_dir: str, formats: List[str]
    ) - Dict[str, Any]:

        self.logger.info(
            f"Exporting trajectories from training results: {training_results_file}"
        )
    ...
    def export_from_evaluation_results(
        self, evaluation_results_file: str, output_dir: str, formats: List[str]
    ) - Dict[str, Any]:

        self.logger.info(
            f"Exporting trajectories from evaluation: {evaluation_results_file}"
        )
    ...
    def _extract_trajectories_from_training(
        ...
        for episode_id in range(min(50, episode_count)):
            ...
            trajectories.append(
                {
                    ...
                    "metadata": {
                        "episode_reward": (
                            training_history.get("episode_rewards", [0])[episode_id]
                            if episode_id
                             len(training_history.get("episode_rewards", []))
                            else 0
                        ),
                        "energy_consumption": (
                            training_history.get("episode_energies", [0])[episode_id]
                            if episode_id
                             len(training_history.get("episode_energies", []))
                            else 0
                        ),
                    },
                }
    ...
    def _extract_trajectories_from_evaluation(
    ...
    def _generate_example_trajectory(
        self, episode_id: int, evaluation: bool = False
    ) - List[List[float]]:
        ...
        for i in range(num_waypoints):
            ...
            x = (
                start_pos[0]
                + t  (goal_pos[0] - start_pos[0])
                + np.random.uniform(-0.5, 0.5)
            )
            y = (
                start_pos[1]
                + t  (goal_pos[1] - start_pos[1])
                + np.random.uniform(-0.5, 0.5)
            )
            z = (
                start_pos[2]
                + t  (goal_pos[2] - start_pos[2])
    ...
    def _export_trajectories(
        self, trajectories: List[Dict[str, Any]], output_dir: str, formats: List[str]
    ) - Dict[str, Any]:
        ...
        for format_name in formats:
            ...
            try:
                files_created = self._export_format(
                    trajectories, output_path, format_name
                )
                export_results["formats_exported"].append(format_name)
                export_results["files_created"].extend(files_created)

                self.logger.info(
                    f" Exported {format_name}: {len(files_created)} files"
                )
    ...
    def _export_format(
        self, trajectories: List[Dict[str, Any]], output_path: Path, format_name: str
    ) - List[str]:
    ...
    def _export_csv(
        self, trajectories: List[Dict[str, Any]], output_path: Path
    ) - List[str]:
    ...
    def _create_floor_plan_visualizations(
        self,
        ...
        floor_trajectories = {i: [] for i in range(1, 6)}

        for traj_info in trajectories[:5]:
            trajectory = np.array(traj_info["trajectory"])

            for point in trajectory:
                floor = int(point[2] / 3) + 1
                floor = max(1, min(5, floor))

                floor_trajectories[floor].append(point[:2])
        ...
        for floor_num, points in floor_trajectories.items():
            ...
            ax.scatter(
                points_array[:, 0], points_array[:, 1], alpha=0.6, s=20, c="blue"
            )

            ax.add_patch(
                plt.Rectangle(
                    (0, 0), 20, 40, fill=False, edgecolor="black", linewidth=2
                )
            )
    ...
    def _create_energy_trajectory_plots(
        self,
        trajectories: List[Dict[str, Any]],
        viz_dir: Path,
        files_created: List[str],
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="data/exported_trajectories",
