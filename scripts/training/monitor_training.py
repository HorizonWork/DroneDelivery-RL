...
class TrainingMonitor:
    ...
    def _get_gpu_usage(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                return {
                    "available": True,
                    "memory_allocated_gb": gpu_memory.get(
                        "allocated_bytes.all.current", 0
                    )
                    / (1024**3),
                    "memory_reserved_gb": gpu_memory.get(
    ...
    def _update_training_history(self, metrics: Dict[str, Any]):
        ...
        if "energy_consumption" in metrics:
            self.training_history["energy_consumptions"].append(
        ...
        if "cpu_percent" in system:
            self.training_history["system_metrics"]["cpu_usage"].append(
                system["cpu_percent"]
            )

        if "memory_percent" in system:
            self.training_history["system_metrics"]["memory_usage"].append(
            ...
            if isinstance(self.training_history[key], list):
                ...
                for subkey in self.training_history[key]:
                    if isinstance(self.training_history[key][subkey], list):
                        self.training_history[key][subkey] = self.training_history[key][
                            subkey
                        ][-max_history:]
    ...
    def _check_alerts(self, metrics: Dict[str, Any]):
        ...
        if "success_rate" in metrics:
            ...
            if (
                success_rate
                < self.monitor_config["alert_thresholds"]["low_success_rate"]
            ):
        ...
        if "memory_percent" in system:
            ...
            if (
                memory_usage
                > self.monitor_config["alert_thresholds"]["high_memory_usage"]
            ):
        ...
        if self.last_checkpoint_time:
            ...
            if (
                ...
                > self.monitor_config["alert_thresholds"]["training_stalled"]
            ):
                alerts.append(
                    f"⚠️  Training may be stalled (no checkpoint for {time_since_checkpoint/60:.1f} min)"
                )
    ...
    def _display_status(self, metrics: Dict[str, Any]):
        ...
        if gpu_info.get("available"):
            print(
                f"GPU Memory: {gpu_info.get('memory_allocated_gb', 0):.1f}GB allocated"
            )
...
def main():
    ...
    parser.add_argument(
        "--experiment", type=str, default=None, help="Experiment name to monitor"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/monitoring_report.json",
