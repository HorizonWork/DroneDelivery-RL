...
Primary training script for energy-aware indoor drone navigation.
Implements complete PPO training with curriculum learning and 5M timesteps.
"""
...
def verify_airsim_connection(retries: int = 5, delay: float = 2.0) -> None:
    """Ensure AirSim is reachable before bootstrapping training."""
    import airsim
...
class MainPPOTrainer:
    """
    Main PPO training system for DroneDelivery-RL.
    Integrates all components for complete training pipeline.
    """
    ...
    def __init__(self, config_path: str, experiment_name: str = None):
        ...
        self.obs_normalizer = ObservationNormalizer(
            observation_dim=40,
            config=self.config.rl.get('normalization', {})
        )
        ...
        if self.curriculum_manager is not None:
            self.logger.info(f"Curriculum phases: {len(self.curriculum_manager.phases)}")
        else:
            self.logger.info("Curriculum manager not initialized - using standard training")
    
    def train(self, resume_checkpoint: str = None) -> Dict[str, Any]:
        """
        Execute main PPO training.
        ...
        # Initialize environment for first/current phase
        self._setup_current_environment()
        
        # Main training loop
        while self.global_timestep < self.total_timesteps:
            try:
                # Check curriculum phase progression  
    ...
    def _setup_current_environment(self):
        ...
        if 'dynamic_obstacles' in phase_config:
            ...
            env_config['obstacles']['dynamic_count'] = phase_config.get('dynamic_count', 3)
    ...
    def _execute_training_episode(self) -> Dict[str, Any]:
        """Execute single training episode with full data collection."""
        ...
        while not done and episode_steps < 1000:  # Max episode length
            ...
            next_observation, reward, terminated, truncated, info = self.current_environment.step(action)
            done = terminated or truncated  # Combine for backward compatibility
            ...
            episode_data['energy_consumption'].append(info.get('energy_consumption', 0.0))
        ...
        self.data_recorder.record_episode({
            ...
            'phase': self.curriculum_manager.get_current_phase()['name'] if self.curriculum_manager else 'phase1'
        })
    ...
    def _advance_curriculum_phase(self):
        ...
        if advanced:
            ...
            self.logger.info(f"🎓 PHASE ADVANCEMENT: {old_phase['name']} → {new_phase['name']}")
    ...
    def _calculate_recent_performance(self) -> Dict[str, float]:
        ...
        # Success rate (reward > 400 indicates success)
        successes = sum(1 for r in recent_rewards if r > 400)
        success_rate = successes / len(recent_rewards) * 100
        
        # Mean energy for successful episodes
        successful_energies = [e for r, e in zip(recent_rewards, recent_energies) if r > 400]
    ...
    def _run_comprehensive_evaluation(self):
        ...
        self.logger.info(f"Running comprehensive evaluation at timestep {self.global_timestep:,}")
        ...
        self.logger.info(f"Evaluation: {success_rate:.1f}% success, {mean_energy:.0f}J energy, "
    ...
    def _report_training_progress(self):
        ...
        self.logger.info(f"Progress: {progress:.1f}% ({self.global_timestep:,}/{self.total_timesteps:,})")
        self.logger.info(f"Episode: {self.episode_count:,}")
        self.logger.info(f"Time: {elapsed_time/3600:.1f}h elapsed, {remaining_time/3600:.1f}h remaining")
    ...
    def _complete_training(self) -> Dict[str, Any]:
        """Complete training and return final results."""
        ...
        history_path = Path(final_checkpoint).parent / f'{self.experiment_name}_history.json'
    ...
    def _load_checkpoint_and_resume(self, checkpoint_path: str):
        ...
        try:
            ...
            if 'curriculum_phase' in additional_data:
                self.curriculum_manager.current_phase_index = additional_data['curriculum_phase']
            ...
            self.logger.info(f"✅ Resumed from checkpoint at timestep {self.global_timestep:,}")
    ...
    def _handle_training_error(self, error: Exception):
        ...
        # In production, might implement error recovery
        raise error

def main():
    parser = argparse.ArgumentParser(description='Main PPO training for drone navigation')
    ...
    try:
        ...
        print(f"Final energy consumption: {results['final_evaluation']['mean_energy']:.0f}J")
