...
Resumes PPO training from checkpoint with proper state restoration.
Handles curriculum phase restoration and training continuation.
"""
...
class TrainingResumer:
    """
    Advanced training resume system.
    Properly restores all training state including curriculum phase,
    normalization statistics, and optimizer state.
    """
    ...
    def __init__(self, config_path: str):
        ...
        # Initialize components (will be updated from checkpoint)
    ...
    def resume_training(
        self, checkpoint_path: str, target_timesteps: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            target_timesteps: Override target timesteps (optional)
    ...
    def _restore_training_state(self):
        ...
        if "normalization_stats" in additional_data:
            try:
                ...
                self.obs_normalizer.mean = norm_stats.get(
                    "mean", self.obs_normalizer.mean
                )
                self.obs_normalizer.variance = norm_stats.get(
                    "variance", self.obs_normalizer.variance
                )
                self.obs_normalizer.count = norm_stats.get(
    ...
    def _continue_training(
        self, target_timesteps: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        Continue training from restored state.

        Args:
            target_timesteps: Override target timesteps
        ...
        self.logger.info(
            f"Continuing training for {remaining_timesteps:,} more timesteps"
        )

        # Initialize environment for current curriculum phase
        current_phase = self.curriculum_manager.get_current_phase()
        environment = self._create_environment_for_phase(current_phase)

        # Training continuation loop
        ...
        resume_results = {
            ...
            "curriculum_phases_completed": self.curriculum_manager.current_phase_index
            + 1,
        }

        self.logger.info(f"Resume training completed in {training_time/3600:.1f} hours")
        self.logger.info(
            f"Final performance: {final_performance['success_rate']:.1f}% success"
        )

        return resume_results

    def _create_environment_for_phase(
        ...
        if "config" in phase_config:
            ...
            if "dynamic_obstacles" in phase_settings:
                env_config["obstacles"]["dynamic_obstacles"] = phase_settings[
                    "dynamic_obstacles"
                ]
    ...
    def _quick_evaluation(
        self, environment: DroneEnvironment, num_episodes: int = 10
    ) -> Dict[str, Any]:
        """Quick evaluation for checkpointing."""
        eval_results = []
        ...
        return {
            ...
            "mean_energy": (
                sum(r["energy"] for r in successes) / len(successes) if successes else 0
            ),
        }
    ...
    def _save_final_checkpoint(self, performance: Dict[str, Any]) -> str:
        """Save final checkpoint with completion marker."""
    ...
    def _log_resume_progress(self, episodes_trained: int, recent_performance: list):
        """Log resume training progress."""
        success_rate = recent_performance[-1] if recent_performance else 0
        progress = self.global_timestep / 5_000_000 * 100

        self.logger.info(
            f"Resume Progress: {progress:.1f}% ({self.global_timestep:,}/5,000,000)"
        )
...
def main():
    ...
    parser.add_argument(
        "--timesteps", type=int, default=None, help="Override target timesteps"
    )
    parser.add_argument(
