...
class HyperparameterSearcher:
    ...
    def search_hyperparameters(self, output_dir: str) -> Dict[str, Any]:
        ...
        study = optuna.create_study(
            ...
            pruner=(
                ...
                if self.search_config["pruning_enabled"]
                else None
            ),
    ...
    def _sample_hyperparameters(self, trial: optuna.trial.Trial) -> Dict[str, Any]:
        ...
        params["rollout_length"] = trial.suggest_categorical(
            "rollout_length", self.search_space["rollout_length"]
        )
        ...
        params["gamma"] = trial.suggest_float(
            "gamma", self.search_space["gamma"][0], self.search_space["gamma"][1]
        )
    ...
    def _train_and_evaluate(
        self, params: Dict[str, Any], trial: optuna.trial.Trial
    ) -> float:
        """
    ...
    def _quick_evaluation(
        self, agent: PPOAgent, environment: DroneEnvironment, num_episodes: int = 5
    ) -> float:
    ...
    def _evaluate_agent(self, agent: PPOAgent, environment: DroneEnvironment) -> float:
        ...
        if successes:
            ...
            energy_score = max(
                0, 100 - (avg_energy / 30.0)
            )  # Scale assuming ~3000J baseline
        else:
            energy_score = 0
