...
class FullCurriculumTrainer:
    ...
    def _train_phase(self, phase_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Target success rate: {phase_config['success_criteria']['min_success_rate']}%")
        ...
        while self.phase_timestep < max_timesteps:
            ...
            if self.phase_timestep % evaluation_frequency == 0:
                ...
                self.logger.info(f"{phase_name}: {self.phase_timestep:,}/{max_timesteps:,} steps, "
        ...
        final_performance = self._evaluate_phase_performance(environment, num_episodes=50)
        
        # Phase results
        phase_results = {
            'phase_name': phase_name,
            'phase_index': self.current_phase_index,
            'success': final_performance['success_rate'] >= phase_config['success_criteria']['min_success_rate'],
        ...
        self.logger.info(f"  Final success rate: {final_performance['success_rate']:.1f}%")
        self.logger.info(f"  Training time: {phase_training_time/3600:.1f}h")
        self.logger.info(f"  Timesteps used: {self.phase_timestep:,}")
        
        return phase_results
    
    def _create_phase_environment(self, phase_config: Dict[str, Any]) -> DroneEnvironment:
        ...
        if complexity == 'low':
            env_config['obstacles']['density'] = min(env_config['obstacles']['density'], 0.10)
        elif complexity == 'high':
            env_config['obstacles']['density'] = max(env_config['obstacles']['density'], 0.20)
            env_config['obstacles']['dynamic_count'] = max(env_config['obstacles'].get('dynamic_count', 0), 5)
    ...
    def _evaluate_phase_performance(self, environment: DroneEnvironment, 
    ...
    def _check_phase_completion(self, phase_config: Dict[str, Any], 
                               success_rates: List[float]) -> bool:
        ...
        return (recent_success_rate >= min_success_rate and 
                episodes_completed >= min_episodes)
    ...
    def _save_curriculum_results(self, results: Dict[str, Any]):
        ...
        phase_summary = {
            ...
            'total_success': all(phase['success'] for phase in results['phase_results']),
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='models/curriculum_training',
    ...
    print(f"  Target Achieved: {'✅ YES' if final_perf['target_achieved'] else '❌ NO'}")
