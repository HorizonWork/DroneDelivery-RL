    ...
    def _setup_conda_environment(self) -> bool:
        ...
        try:
            ...
            if self.conda_env_name in result.stdout:
                self.logger.info(
                    f"Conda environment '{self.conda_env_name}' already exists"
                )
                return True
            ...
            self.logger.error(
                "Conda executable not found. Please ensure conda is properly installed and in system PATH."
            )
            return False
    ...
    def _create_example_configs(self):
        ...
        main_config = {
            "environment": {
                ...
                "obstacles": {"density": 0.15, "dynamic_obstacles": True},
            },
            "rl": {
                "ppo": {
                    "learning_rate": 3e-4,
                ...
                "training": {"total_timesteps": 5_000_000, "eval_frequency": 50_000},
            },
            "evaluation": {"num_episodes": 100, "timeout": 300.0},
            "logging": {"level": "INFO", "console_logging": True, "file_logging": True},
        }
        ...
        eval_config = {
            ...
            "visualization": {"save_plots": True, "figure_size": [12, 8], "dpi": 100},
        }
...
def main():
    ...
    if args.force:
        try:
            subprocess.run(
                ["conda", "env", "remove", "-n", args.env_name, "-y"],
                capture_output=True,
            )
