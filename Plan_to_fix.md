# Plan_to_fix.md — DroneDelivery-RL (branch `dev_minhky`)
...
## 2. Checklist các vấn đề & hướng xử lý

### 2.1. Drone spawn & reset (CRITICAL)

#### [x] 2.1.1. Thống nhất `spawn_location` & `spawn_orientation`
...
- **Hướng sửa**  
  ...
  2. Trong file config:
     ...
     spawn_orientation: [0.0, 0.0, 0.0]  # pitch, roll, yaw
     ```
...
#### [x] 2.1.2. Đảm bảo đúng hệ toạ độ & độ cao (NED)
...
#### [x] 2.1.3. Thêm bước **takeoff** trong quá trình `reset()` khi train
...
- **Hướng sửa (gợi ý)**  
  1. Trong class env, sau khi `reset_drone(to_spawn=True)` thì:
     ```python
     def reset(self):
         # 1. Reset drone về spawn
         self.bridge.reset_drone(to_spawn=True)

         # 2. Takeoff & lên độ cao mong muốn (vd. 3m)
...
#### [x] 2.1.4. Kiểm tra & chuẩn hoá `spawn_orientation`
...
#### [x] 2.1.5. Xử lý an toàn `spawn_randomization` (nếu đang dùng)
...
### 2.2. Spawn vật thể (tường, phòng, dynamic obstacles, goal)

#### [x] 2.2.1. Xác nhận lại alignment giữa world builder và AirSim
...
#### [x] 2.2.2. Kiểm tra vùng spawn của dynamic obstacles
...
### 2.3. Training loop & curriculum

#### [x] 2.3.1. Xác nhận điều kiện đổi phase curriculum
...
### 2.4. Algorithm & reproducibility

#### [x] 2.4.1. Sync lại observation & reward với report
...
#### [x] 2.4.2. Sync hyperparameters PPO
...
#### [x] 2.4.3. Thêm random seed để reproducible
...
#### [x] 2.5.1. Viết script test spawn + takeoff (smoke test)

- **File đề xuất**  
  - `scripts/debug/test_spawn_and_takeoff.py`

- **Ý tưởng**  
  - Tạo bridge + env, gọi `reset()` 5–10 lần, chỉ lấy obs và đợi vài giây.  
  - Mục tiêu: mỗi lần reset drone đều spawn đúng chỗ, lên đúng độ cao, không từ từ rơi xuống.

#### [x] 2.5.2. Log trạng thái ban đầu của mỗi episode
